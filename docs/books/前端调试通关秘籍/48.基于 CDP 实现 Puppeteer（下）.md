...
![](./images/188560a79c2b417a8b08d0d961cfc84f~tplv-k3u1fbpfcp-watermark.image.png)

在 chrome devtools 里打开 Protocol Monitor，就可以看到 CDP 的数据：

![](./images/e7e2fb9a95c94ab2b6407323d619e4fc~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/615b67e2aa6242969a7c839be0df247c~tplv-k3u1fbpfcp-watermark.image.png)

它是分为不同的域的，比如 Page、Browser、Network 等，分区来管理不同的协议。

比如 Page.navigate 可以让页面导航到某个 url：

![](./images/bf87dd5608c24a479598f7f2193c3cfb~tplv-k3u1fbpfcp-watermark.image.png)

Page.close 可以关闭页面

![](./images/8d297a4308364921b105401dc6d8efd6~tplv-k3u1fbpfcp-watermark.image.png)

Browser.close 可以关闭浏览器

![](./images/aaad0818b07747e992703a2ff2c1818c~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/278bdea4fb364b48a7c998ff979f9b7d~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/dc146147afb2499892a3a4d97220e229~tplv-k3u1fbpfcp-watermark.image.png)

但这个参数在低版本的 chromium 不支持，所以如果有报错就用版本高一点的 chromium 来跑，比如我这里用的是 970501

![](./images/cbb19dc6d70047feb9e75dbc848b0693~tplv-k3u1fbpfcp-watermark.image.png)

以调试模式跑起 Chromium 之后，访问 http://localhost:9929/json/list 就可以看到每个页面的 ws 服务的信息，可以连上每个页面进行调试：

![](./images/7a81c9a69eba4fc4aef43f70074a295a~tplv-k3u1fbpfcp-watermark.image.png)

比如我再访问下 baidu 和 juejin，就会多这俩页面的 ws 调试服务的信息：

![](./images/b6c7c11021814e9ea956ebaf41f97b98~tplv-k3u1fbpfcp-watermark.image.png)

我们可以用 http://localhost:9929/json/list 这个页面是否可以打开来判断浏览器是否以调试模式启动成功了。

然后你还会发现 /json/new 可以新建一个页面：

![](./images/4ace6a5aac4c4c71b953938ae44986c4~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/1cf0642a18a240d187c9e07ad535f9e7~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/e2f6e472c8cf455b8fb965d9721023f3~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/e5184adff3a84400b7aa1b4478e6b61c~tplv-k3u1fbpfcp-watermark.image.png)

之后开始做一些页面级别的控制：

CDP 每个域的使用都要先开启下，创建 Page 对象的时候我们已经开启了 Page 域的协议：

![](./images/dc1af34b69c740a999634637fe7247c4~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/6b8340819627488e9ae0d6d5182b177a~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/3c568de6b3fc4692bc0533bb17d12dfe~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/59a5432d92194024afdff7922b67eecc~tplv-k3u1fbpfcp-watermark.image.png)

把 baidu 页面的背景改为粉色，并且拿到热榜列表的文本数据。

测试下：

![](./images/0f2e328400e24433a55adf3b0151c934~tplv-k3u1fbpfcp-watermark.image.png)
