...
![](./images/a94ab697beec45328b6ff70440dd8550~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/979ceb2ff4ff4ddd80c6f01d5a0a0b22~tplv-k3u1fbpfcp-watermark.image.png)

在 sources 面板里可以直接看到哪些代码没执行，比如下面的红色部分就是没有执行的：

![](./images/33e07a0cfbaf4052b26502e3e458be66~tplv-k3u1fbpfcp-watermark.image.png)

这个功能还是很有用的，可以帮助我们分析哪些代码是用不到的，可以进行延后加载或者删掉等优化。

在 More Tools 里开启：

![](./images/199841149c8f449980a86b82ea9fef73~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/be456787fc464db19c707048f8d0e216~tplv-k3u1fbpfcp-watermark.image.png)

事件对象里是这段 js 的 url 和行列号，再就是 scriptId。

然后再看下 CSS.styleSheetAdded 的事件对象：

![](./images/0fdc216537454a26bfc8b4093859ebeb~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/b07fbb4a4e3e480384acc5dec8941fe2~tplv-k3u1fbpfcp-watermark.image.png)

![](./images/7a985bad3cb14e8699829d161c9d5cf4~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/22edc8011a3342049131e659722f91c5~tplv-k3u1fbpfcp-watermark.image.png)

返回的结果显示 scriptId 为 89607.4 的 css 的 50 到 80 个字符的代码执行了。

我们在 cssMap 里看下这个 id 对应的代码：

![](./images/5de8f5c77de0491bb11b12ab44538a36~tplv-k3u1fbpfcp-watermark.image.png)

然后取出 50 到 80 个字符的代码：

![](./images/eef0cbc359694b65a7033779ec719304~tplv-k3u1fbpfcp-watermark.image.png)

也就是说所有 css 里只有这一段代码是生效的：

![](./images/5aedc6f0744e4e7e922b86a470784769~tplv-k3u1fbpfcp-watermark.image.png)

你用 Chrome DevTools 的 Coverage 分析结果也是这样的：

![](./images/e7bd66759e6544c2aed5b96d0e0ab30f~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/2f6f52424b594b059d5bb599459e6db8~tplv-k3u1fbpfcp-watermark.image.png)

因为我们页面上就两个 script 嘛：

![](./images/a6b4b3a6707d410185dd9931fdcb8e34~tplv-k3u1fbpfcp-watermark.image.png)

第一个 script 有 4 个 functions：

![](./images/502a0763a7fa467488fbda9661aceac3~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/df3d7109b5b940b8ab8cd657ac6854cc~tplv-k3u1fbpfcp-watermark.image.png)

minus 函数执行了 0 次：

![](./images/83795c826da84c3e830bf338a2fed2af~tplv-k3u1fbpfcp-watermark.image.png)

第二个 script 的匿名代码块执行了 1 次：

![](./images/5fff78cf57ec429685689f6419561ba1~tplv-k3u1fbpfcp-watermark.image.png)

这不就和 Chrome DevTools 的 Coverage 结果对上了么：

![](./images/f96447901cc04c1d9c47d5f280d5e721~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/d16c2fee307a40a68fb6a427d6f1143c~tplv-k3u1fbpfcp-watermark.image.png)
