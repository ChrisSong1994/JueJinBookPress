...
![](./images/6a02ad15e2504619920e06730a00fedb~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/2d890fa5e94e49beb7b296589afb4452~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/c646a1a3d6e346dab7cc4ed507997ba7~tplv-k3u1fbpfcp-watermark.image.png)

选择一个 npm 包管理工具，然后 nest cli 会自动创建项目结构并安装依赖：

![](./images/2e06a93b361f49888d95bd78a05b55eb~tplv-k3u1fbpfcp-watermark.image.png)

然后进入这个目录，执行 npm run start，把服务跑起来：

![](./images/fb4b95248dbd4a9493e6b3e05d407f1a~tplv-k3u1fbpfcp-watermark.image.png)

浏览器访问 http://localhost:3000 就可以看到 nest 服务返回的 hello world：

![](./images/60b7f8ba57a84eebaf895b818a494416~tplv-k3u1fbpfcp-watermark.image.png)

我们看看代码里它是怎么创建对象的：

![](./images/08429e4340cf4baf851b2bacb1359cd0~tplv-k3u1fbpfcp-watermark.image.png)

它有一个 AppService 声明了 @Injectable，代表这个 class 可注入，那么 nest 就会把它的对象放到 IOC 容器里。

![](./images/362872afddcc4a6d8b45e73926c95fe7~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/1145a8a972404fc9a68a5207f7a236bd~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/6bddbc6b4da34e87bfbaea8245c4b634~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/b8862d45d9c04adc98230c81042e9b4a~tplv-k3u1fbpfcp-watermark.image.png)

那么 nest 就会从 AppModule 开始解析 class 上通过装饰器声明的依赖信息，自动创建和组装对象。

![](./images/5245830454164037a3001c6c4288a8b3~tplv-k3u1fbpfcp-watermark.image.png)

所以 AppController 只是声明了对 AppService 的依赖，就可以调用它的方法了：

![](./images/1a0ad84c4cec43bc8c0bbbc2de99e8f5~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/2ee471d1a60e4c428046a2dac1ff0b82~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

会生成如下代码：

![](./images/d33f113a003b4738a5642240df83535e~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

用 nest cli 的 generate 命令生成一个模块。

会在 AppModule 里自动 imports 这个模块：

![](./images/49df422f1ac842f3b3b1f27089ef029f~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
```
![](./images/41684f4682384465bc0a7697318bdbc8~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

会生成 Service 的代码：

![](./images/484501bfb1404c55a4ac311183782541~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

并自动添加到 OtherModule 的 providers 中：

![](./images/fc073d7985964d66ac58d02ad1dd50e4~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

我们改下 OtherService，添加一个方法：

![](./images/5c35a8d474b44d029d8ebbd465e22b7a~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

然后在 OtherModule 里 exports：

![](./images/14a2fe1b44ed4373a1c619a8ba69a679~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

那当 AppModule 引用了 OtherModule 之后，就可以注入它 exports 的 OtherService 了。

![](./images/7166b487dde24df6988e8358c0b1cd23~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
npm run start:dev
```
![](./images/110accc85c7a409395c52f33f44ba9ce~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)


浏览器访问下，可以看到 AppModule 的 AppService 调用 OtherModule 的 OtherService 成功了：

![](./images/e6bea726dbab477da9cbeca914e296b2~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
