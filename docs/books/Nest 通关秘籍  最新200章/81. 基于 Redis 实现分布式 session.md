...
![](./images/c5298508d5b74d4585c58b0034a96125~tplv-k3u1fbpfcp-watermark.image.png)

而 jwt 是把所有的用户数据保存在加密后的 token 里返回，客户端只要在 authorization 的 header 里带上 token，服务端就能从中解析出用户数据。

![](./images/95fd322ae4ec4b2bba3973405a659de4~tplv-k3u1fbpfcp-watermark.image.png)

jwt 天然是支持分布式的，比如有两个服务器的时候，任何一个服务器都能从 token 出拿到用户数据：

![](./images/c2d4af048e3e4c1f965b23f6184f7a70~tplv-k3u1fbpfcp-watermark.image.png)

但是 session 的方式不行，它的数据是存在单台服务器的内存的，如果再请求另一台服务器就找不到对应的 session 了：

![](./images/76f00b76e0d944d584927aa6e913ba0d~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/c92708cac3374e99a3e4a813bcfe7fe3~tplv-k3u1fbpfcp-watermark.image.png)
...
```
![](./images/875d542bb4a1407f90c43798f4fe6098~tplv-k3u1fbpfcp-watermark.image.png)
...
```
![](./images/2c8d0a8556f24180825e213dd36958b2~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/af5a3f5a84854fa9aa7f9af167eb7575~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/29816011bc1741f49df6715d3513503a~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/a881300cf443467e8540debaa894ec23~tplv-k3u1fbpfcp-watermark.image.png)

现在 getSession 返回的是 Record<string, any> 也就是对象类型，但并不知道有啥具体的属性。

![](./images/4c14008ddf454a5ea7f0b823b1aed9ac~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/1be5e6fc8e0142cb8212d9089b2ecde4~tplv-k3u1fbpfcp-watermark.image.png)

传入类型参数之后，返回的就是该类型了：

![](./images/13e8b607cf6244ffb9fcb78770892ec1~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/09b4d078662e4fc2908d1e284d4866d1~tplv-k3u1fbpfcp-watermark.image.png)

我们自己实现的 session 就生效了：

![](./images/faaa9f20ae824c8294941553cd3f39e5~tplv-k3u1fbpfcp-watermark.image.png)

在 Redis Insight 里可以看到 session 的值

![](./images/5ee658369fbd4a0cb212cf4f2aefd361~tplv-k3u1fbpfcp-watermark.image.png)

而且这个 session 是支持分布式的。


![](./images/991dd7dd3a6440d1bb63999a63bb2881~tplv-k3u1fbpfcp-watermark.image.png)
