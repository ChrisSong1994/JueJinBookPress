...
![](./images/f8a3cc61bbaf46d282925619ae1d6776~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/488d4088be5c474695d46a0d7de80209~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

在 index.tsx 里把这三行代码注释掉：

![](./images/9e26daa6a3af49e79d6c15fa42a4d3b6~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/919e3c8f17374ae89d2c543d4228167a~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/a755c117d7844cc9adeec24c9d6f7d35~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

state 初始值是 1，点击改变状态，这个很简单。

如果初始状态需要经过复杂计算得到，可以传个函数来计算初始值：

![](./images/a9afbbcd4bb94a5294ff77328a5601b9~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/ca5d92cfb59d4bddb102b46b9f3fda2f~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

可以直接传新的值，或者传一个函数，返回新的值，这个函数的参数是上一次的 state。

跑一下，功能正常：

![](./images/6549efeb030a4e2db5d209c6d4880cfc~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/ec2ec4d228ef4723a1c3b34c1912d070~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/b23136f3505244bdbbbda70cf15bf470~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

浏览器访问下，可以看到 2s 后，state 变为了 666:

![](./images/ccb47b6b9ba3439e9f3e1317b5fb337f~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

请求数据、定时器等这些异步逻辑，我们都会放在 useEffect 里。

回过头来看下 useEffect：

![](./images/b659131995a94c539be88d7a10eec30a~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/682df1ade6944f93bb32c738684c8574~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

现在这个组件会渲染两次，初始渲染和 2s 后 setNum 触发的渲染，也就是 function 会执行 2 次。

打开 devtools 看一下：

![](./images/6445a28abe5a4272b51375ad9773b66b~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

xxx 只执行了一次，因为 [] 每次都不变。

我也可以写任意的常量，因为它们都不变，所以不会触发 effect 的重新执行：

![](./images/2a62909c110e47da921c9491a2d872bf~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

![](./images/aa31e59163e14b07a84ea9fa10445d72~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

但如果其中有个变化的值，那就会触发重新执行了：

![](./images/d3b4df1b9868480d8db12f509ef8a279~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

可以看到，现在每次渲染，effect 都会执行：

![](./images/943f10aa908a4d73b9452cf7b02734f0~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

这个数组我们一般写依赖的 state，这样在 state 变了之后就会触发重新执行了。

不传 deps 数组的时候也是每次都会重新执行 effect 函数：

![](./images/1afe9118d4334dcd823b32d41b8341b1~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

![](./images/89023cde2d314ceb97487dc3e13e998f~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

那 useEffect 里如果跑了一个定时器，依赖变了之后，再次执行 useEffect，又跑了一个，怎么清理上一个定时器呢？

这样写：

![](./images/fa390c7514404adda7a6eb758192f44a~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/a83277f6c3c74154986fcecd2cab7538~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/3f1898ab718041f794f97fb3eaff45cc~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

![](./images/9051c70096bf4e9d9053d6d5723c3b40~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

那为啥还要有这两个 hook 呢？

我们知道，js 执行和渲染是阻塞的：

![](./images/9c88835aa7434d15ae1b60d77aff4215~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

useEffect 的 effect 函数会在操作 dom 之后异步执行：

![](./images/a554b4ddeb494d69991f8c71e543d72a~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/c724f011d3f549678cc1b4c85a669b96~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

可以看到，渲染的间隔是固定的，而 js 的任务在这些渲染的间隔中执行。

所以异步执行的 effect 逻辑就有两种可能：

![](./images/83b2167ca07b464c88af9b5e3df46093~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

![](./images/8a1e6d47466747478556c6235df5d27a~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/c7352b1c05bc4bf2862b2297e4303c8b~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/d8ca8b96386d43eb94ddabe5f277d195~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/07d4658f8c9a4a55bd376d645413c512~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

![](./images/9e262d5d01304812b036b7abc5353888~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/e6e76469d96e4be49e1cafebe7398de4~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/fb65b8784ea34ef5ae169c54a0c3af4a~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

如果你直接修改原始的 state 返回，是触发不了重新渲染的：

![](./images/fd6bb26f55004e22b9829a2afc9bca55~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/6fa4294aa3cd4f52903347e5ad33a64c~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/74c7cb0691684797846a97e34d82cbf0~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/2e96c9dca37640b49395bdec1416ac8c~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/3fc79973dcb14bc68292fd02aa6b2f3e~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

也可以用 immer：

![](./images/742e8dd844334ad78b7d51bcbb176d18~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
```
![](./images/8c06eee6198548ca93691db984b911ec~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/b1c73d023f204a198f55091e808c80dc~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

可以看到，focus 生效了：

![](./images/354bc73ea70c49909216aa038544e374~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/f2ac156fa9bc402ca26b6b740ac87299~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

想触发渲染，还是得配合 state：

![](./images/1ede3fb6868e4f34bd71adae6b689bd9~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/b383a6f03f8d4204829e977d95267011~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/44f1ec8a3d5841b48f59d49363bd2f2e~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/7102c90ebd7e4f1da3da7ee2401f3850~tplv-k3u1fbpfcp-watermark.image.png)

这样就把子组件的 input 的 ref 传递到了父组件。

效果如下：

![](./images/4752b25fec3e4ca18834bc8e6090a169~tplv-k3u1fbpfcp-watermark.image.png)

不过被 forwardRef 包裹的组件的类型就要用 React.forwardRefRenderFunction 了：

![](./images/cc27ede261f143fa80038a90b924917f~tplv-k3u1fbpfcp-watermark.image.png)
...
![](./images/ae40f5488f01445698e126456caa5dc6~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

这样，父组件里拿到的 ref 就是 useImperativeHandle 第二个参数的返回值了。

效果是一样的：

![](./images/2ea0d65a5f18419f9f2661ed5dc04daa~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

## useContext

跨任意层组件传递数据，我们一般用 Context。

![](./images/a0e7034890d84ab18627c430d53a9dc8~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

创建 App5.tsx

![](./images/eba5220484a34efeb9a9704655137f29~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/5a8157bc169b4e3298a86d4289879e9c~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

访问下：

![](./images/e488bf6cfe7e499dba551b1b780b7e13~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/72d28d315a5a46c5ac219deaf87bae42~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/a1554790572244c88d695a48eb98ed72~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/818a6bbc28444642abdecdeb043d0aa3~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/e31e9f4b0a674f4cb0aa4a3c22091658~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

我们让 2s 后 props 变了呢？

![](./images/5e9c5f674eea47b6bc1e1896646db839~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/454fb44ba8ce476592f4a0c2d90dee71~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/ddfb932af2184ce48255446d346fe6b6~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

参数传了一个 function，你会发现 memo 失效了：

![](./images/f203f20743c34905ba6148b328d8bbc2~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

因为每次 function 都是新创建的，也就是每次 props 都会变，这样 memo 就没用了。

这时候就需要 useCallback：

![](./images/7d02270a19374897a3a8e27f2f547503~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
...
![](./images/17318aee450b4918b59db4e000272922~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)

同理，useMemo 也是和 memo 打配合的，只不过它保存的不是函数，而是值：

![](./images/29d8c121a66f472b8de743b4784abcfa~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image.png)
