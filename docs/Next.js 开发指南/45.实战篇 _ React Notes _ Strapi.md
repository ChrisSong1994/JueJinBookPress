...
![image.png](./images/56047c198fecdb30166d9b5c8e6621f5.webp )
...
![image.png](./images/98683e395feb3a92f1933df1e46d52fd.webp )
...
![image.png](./images/ddfb9af58c34085f1fa8f908d1a48bfc.webp )
...
![image.png](./images/3ba56060bd1dcc81201185de8236f7af.webp )
...
![image.png](./images/d378f69b4a8dbbab469f2567a84dcfe8.webp )

如果系统是 macOS 11，可以选择 8.0.28 版本：

![image.png](./images/37b26e00eb1e4780b57cd01aa64ca9bd.webp )

安装的过程中需要设置下 root 用户的密码，记住这个密码就行。

安装完成后，可以在“系统偏好设置”中查看到：

![image.png](./images/b8672dac0a8344279cb5f2a4061b623a.webp )

点击进入 MySQL 界面，点击 `Start MySQL Server`即可启动 MySQL：

![截屏2024-01-16 下午1.30.21.png](./images/163462ce0250944c1d0dfa32920eda93.webp )
...
![image.png](./images/69cb8307f1760992a8c146557cb5ab99.webp )
...
![image.png](./images/9aaec7e90ca97367c186cef60830b92a.webp )
...
![image.png](./images/6aff77e66c7a0daa5db938addc1d82ca.webp )
...
![image.png](./images/c145e2ab9648f6313c3d9db272832e98.webp )
...
![image.png](./images/03e1c8f7e3ca26f2f24b6eaa636c318a.webp )

这里的信息用于 Strapi 认证，所有的数据也都存储在本地的数据库里。所以这里随便填，但是得记住，以后可能会用到。

填写完后进入主界面：

![image.png](./images/5014558df911fdb3b2d8aab30224a432.webp )
...
![image.png](./images/d5b412f9a1a3a97852bd1d8974385b31.webp )

保存后，主界面即改为中文：

![image.png](./images/6e2dac801278dd0f6b6c4101173f36d8.webp )
...
## ![image.png](./images/1b42135ec175383008e0c3d6de300aba.webp )

然后就是添加各种字段，对应为表建立各种字段：

![image.png](./images/d3166b4bacb3ee1ed41f60875da116d9.webp )

#### 2.2. 填充数据

回到 `Content Manager`，选择 `Note`这个集合类型，然后点击 `Create new entry`，这步就是让你填充一些数据。我们象征性的填充一些数据：

![image.png](./images/7676c5adc9c1cdc457aea96bd1492f78.webp )

#### 2.3. 生成 token

打开 `Settings` -> `API Tokens`，点击 `Create new API Token`，生成 API Token，该 Token 决定了权限范围和使用时间。

![image.png](./images/92220104a011e2f837535cef38bde5e1.webp )

生成之后，获取接口数据的时候就需要带上这个 token：

![image.png](./images/169515cb45c1594ba778b4208016886d.webp )
...
![image.png](./images/3e8bc7b4e88f335d8e8f10625aae9907.webp )

如果你不带 token 获取就会出现 403 错误：

![image.png](./images/914750a99e6380786d1a6f2be21c4512.webp )

#### 2.5. 取消授权

那你可能会想：“好麻烦，我调用个接口，还要用 token，能不能不用 token，至少获取列表和获取条目不需要？”。当然也是可以的，我们点击 `Settings`-> `Roles`，选择 `Public`角色进行编辑：

![截屏2024-01-16 下午9.10.58.png](./images/c9f451e4e928c52c7ab095049272f05f.webp )

勾选 Note 这个集合类型中的 `find` 和 `findOne`，表示 `/api/notes`和 `/api/note/1`不再需要鉴权。

![image.png](./images/c35b8c8a9bdf456c84e75378b4339385.webp )

现在我们已经可以直接获取数据：

![image.png](./images/29d1d41ff8bf6c19ceeafc1f14d87e57.webp )
...
![image.png](./images/0ae353fed9f56fc60dc9eeb2b4f8bc2e.webp )

但这个 UID 生成的字符串是 `note`、`note-1`、`note-2`这种。我们希望是一个多位的随机数字字符串。该如何实现呢？

这就要说到 Strapi 强大的插件功能了，我们打开 Marketplace，搜索 `uuid`：

![image.png](./images/fe87475de6cb50f9f0604a90847b8e95.webp )
...
![image.png](./images/30fc2c555cc390c4b9d3061871ee5d02.webp )

我们建立一个名为 slug 的 UUID 类型，UUID format 表示这个 uuid 的格式，我们填写 `^[0-9]{8}$`表示随机的 8 位数字字符串。

![image.png](./images/22c94a51e420e860fffed7c24bd098ce.webp )

我们就可以通过该字段添加随机的 uid 数据：

![截屏2024-01-16 下午10.05.56.png](./images/991d6560ee1c738ac73a290af1cae066.webp )
...
![ReactNotes-Auth9.gif](./images/d09b83990a05857e6b30b31ae6a3d4b2.gif )
