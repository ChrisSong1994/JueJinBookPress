...
![image.png](./images/06a42923556977e4353f11dc7b4eeb81.png)

当点击具体笔记的 `Edit` 按钮的时候进入该笔记的编辑页面：

![image.png](./images/d7db0dd26befbeb8bd3544049631833e.png)
...
![image.png](./images/d19f44cd0ca7812dadc21e5e16760503.png)
...
![截屏2023-12-18 下午4.57.17.png](./images/b6671487cff01e1e1e3dc7dbd16aee09.png)

预览的时候，我们虽然用了 `<NotePreview>` 这个组件，但是代码没有打包到客户端中。但是当我们打开 `http://localhost:3000/note/edit/1702459188837`：

![截屏2023-12-18 下午4.58.35.png](./images/0258082aa36a355d20f5f20cfa627a17.png)

你会发现，下载了客户端组件 `<NoteEditor>` 和 `<NotePreview>`，对应也使用了很多库。`page.js` 也变大了很多（424 kB）：

![截屏2023-12-18 下午5.02.22.png](./images/6b82620e2daeeece4382d27d7188706b.png)

最后再说说使用客户端组件时的一个注意事项，那就是不要使用 `async/await`，可能会出现报错：

![image.png](./images/ed9a510ca96ee3719053d126b203b229.png)
...
![image.png](./images/20f5fd56cc7d924714e34cc6d7ee3dce.png)

请求地址是当前页面，请求方法为 POST。请求内容正是我们传入的内容：

![image.png](./images/0e2a0935d9c0aedfa29e16f38609373e.png)

响应内容为：

![image.png](./images/7506643071a67f8db9f1c2a6a83ebb58.png)

如果我们不注释掉 `actions.js` 中的 `redirect`，然后我们编辑一条笔记，然后点击 `Done`，可以看到页面发送了一条 POST 请求：

![image.png](./images/3beb326095999a325590a71d8c0b01bc.png)
...
![image.png](./images/c2e075a0a6d0b221f7a5a597b9fbe6d6.png)
...
![image.png](./images/4e9e91f191c134f8914cd69684e6600b.png)
