...
![image.png](./images/3d36c84f91fc49d4d1c2ab31046b90b4.webp )
...
![image.png](./images/177f174e2df6ceb202fe4b4d8f028262.webp )
...
![image.png](./images/c07f41dcd9d932fc3db61a402ef2d19e.webp )

当部署完成后会进入以下页面：
![image.png](./images/307bf99c96d5b90a0241d3939ee60ce3.webp )

在 `Dashboard` 页面点击 `Visit` 按钮即可访问部署后的效果：

![截屏2024-02-22 14.51.58.png](./images/36ec02710a866bc34e6b0221846069ed.webp )

像我这次的部署地址为：<https://next-app-demo-ebon.vercel.app/>，效果如下：

![image.png](./images/af3b7add7b421f7af7b95404cfad74e2.webp )
...
![image.png](./images/d0b3c7087845d7b4eb62ba74b1fcd397.webp )

其中记录值填写我们项目部署的域名。

最后在 Vercel 的设置中添加设置的域名：

![截屏2024-02-22 15.58.59.png](./images/bab0d0f0c8b48f9e510fdba6c2222bb7.webp )

效果如下：

![image.png](./images/ad4049186bacf6b6564ee9743c17fcf3.webp )

#### 国内访问

Vercel 直接部署的域名是无法访问的，我们可以通过 <https://tool.chinaz.com/dnsce> 检测我们的域名：

![image.png](./images/4600f9de02b0f0b30bde8135e8b2f79e.webp )

如何让国内用户也可以访问呢？可以修改我们的域名解析：

![image.png](./images/77d750653329ff0f0eeefb57510e6001.webp )
...
![image.png](./images/86a5e99ddb2aae382aaf8e73b242a732.webp )
...
![image.png](./images/0c7b2700bf59ed28f506b64c256974fc.webp )
...
![image.png](./images/1eb38e05e0d28137c5e85d8033d14d58.webp )
...
![image.png](./images/9040ae845be1e09d4e42ef83a0cc0b53.webp )
...
![image.png](./images/0c9c63f283f004ecf76b66e15e2c2ff2.webp )

#### 创建数据库

通过刚才的步骤建立一个项目后，在 Vercel 平台进入创建的项目，选择 `Storage` 选项，这里展示了 Vercel 目前支持的四种数据库：

![截屏2024-02-22 21.06.15.png](./images/9077bc84c87dd0600217ddee9b652911.webp )
...
![image.png](./images/b35705d535839f803ef32ab4823a5fb8.webp )

继续点击 Connect：

![image.png](./images/c908f51ed2f1b24c8c5603c4c0e12bc6.webp )

现在可以看到我们创建的 Redis 数据库的地址，Vercel 也贴心的提供了接下来要做的事情：

![image.png](./images/d09934a9bd20b9124805fe6b9f862d46.webp )

我们解释下这些要做的事情：

1.  Connect to a project：将已有的 Vercel 项目与该数据库进行关联，刚才已经点击了 Connect ，所以不需要再点了。Connect 后，Vercel 会为项目自动添加数据库相关的环境变量：

![截屏2024-02-22 21.46.30.png](./images/c3001eb091b665d4000dcf664e6e3423.webp )
...
![image.png](./images/54a119d07338776fe6a5ba875dca9acf.webp )
...
![image.png](./images/d6466704797ce6a1aa32df897cbd71a6.webp )

因为服务端的环境变量还没有建立，我们在该 Vercel 项目上添加一个新的环境变量 `REDIS_URL`：

![截屏2024-02-23 16.50.54.png](./images/aa10d7af504a56b7171bbe6dda8ee5fe.webp )

然后重新部署，点击 Redeploy：

![image.png](./images/2a4289505dc93ed8128ec097bfae847f.webp )

此次应该会成功部署：

![image.png](./images/9da56686ad54cd9fcd49c0ed083e61d5.webp )
...
![截屏2024-02-23 17.11.29.png](./images/6d4097b8649fc765c8f0d97db15f8277.webp )
...
![截屏2024-02-23 17.16.03.png](./images/01eff846245a537c43bf5ed2b8dec3c0.webp )
...
```bash
# 50.实战篇 _ React Notes _ Vercel 部署
npm i && npm run dev
```
...
![image.png](./images/06eadf41f1f25c7a76bd7160c22303cb.webp )

建立项目后，选择 `Storage`选项，建立一个 Vercel Postgres 数据库，最终获得该数据库地址为：

![image.png](./images/cb213e7e3e91a24d63cb2f5c683b1d77.webp )

拷贝上图 `.env.local` 选项中的环境变量，将其写入 `.env`文件：

![截屏2024-02-26 17.50.19.png](./images/a5e16e3fe7f62d615100bb4114f275da.webp )
...
![image.png](./images/d7ec4193c57da75ff4b96db0095365f0.webp )
...
![image.png](./images/ccb93bf125c74b3407b16c5a394eace8.webp )

目前我们还在 day11 分支，切换为新的 `vercel-postgres`分支，然后将代码提交到远程的 `vercel-postgres`分支，因为是首次部署，所以会部署到生产版本，交互效果如下：

![image.png](./images/8794138ee4aa8d65ded44eaa4978b830.webp )

Vercel 自动部署后，效果如下：

![image.png](./images/92d45404e64c2869629340870b625cc8.webp )
