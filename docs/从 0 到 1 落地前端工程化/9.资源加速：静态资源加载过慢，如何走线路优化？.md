...
![个人官网](./images/a33f144bc509764268af09a9d262c4fb.webp )
...
![缓存机制](./images/1d1d6a0dd578290c712aadaa950648fc.webp )

##### 缓存策略

`缓存策略`通过设置`HTTP报文`实现，在形式上分为**强缓存(强制缓存)**与**协商缓存(对比缓存)**。为了方便对比，通过下图展示它们各自的细节，相信你有更好的理解。

![强缓存](./images/dfc1fc52c8be444460a2ee194b2537f7.webp )

![协商缓存](./images/ad7809c6f4810c2d914c0a94c85ee165.webp )
...
![京东物流](./images/ffa3b1c227d3b7f96d02629df675e826.webp )

同理，`CDN`的工作原理是将源站的资源缓存到各个`CDN节点`中，当请求命中某个节点的资源缓存时，立即将资源返回到客户端，避免每个请求都通过源站获取。

![CDN节点](./images/70773bb7432450dc0920d3245eeaf84e.webp )
...
![添加域名](./images/997c8ab15e52c2204164221d38b1304f.webp )
...
![添加域名-域名信息](./images/50de8de7a4f1ba400f1b4fc3470b43da.webp )
...
![添加域名-域名信息-新增站源信息](./images/7119033779322f052106ac9cedf63dfd.webp )

填写或选择必填信息，拉动网页到底部点击`下一步`。

在`推荐配置`中不做任何填写或选择，拉动网页到底部点击`下一步`。

![添加域名-推荐配置](./images/389baf8a24e3bbaee195a70ed811752c.webp )

在`CNAME配置`中出现以下提示则证明配置成功，但还需根据提示完成加速域名的`CNAME配置`。

![添加域名-CNAME配置](./images/ee97a0cffbb8bba58d0c910eb35af73c.webp )
...
![证书文件](./images/b1ff9eea0dd8da016b681299c3d3bee3.webp )
...
![修改记录](./images/667602d74898e7f9bafc6e5d639c0245.webp )
...
![确认开启HTTPS](./images/37985d276050599b52625cf61f313130.webp )

打开[阿里云官网](https://www.aliyun.com)，选择`右上角的控制台 → 左上角的菜单 → CDN → 概览 → 购买`。个人官网选购`1年1000万次`的`静态HTTPS请求包`就够用了，打折后`32元`。

![购买](./images/0bc501bd6e739a901fc0c3dc7886e07d.webp )

![静态HTTPS请求包](./images/3b901f81d5a2fdcc06baeef0d595e495.webp )

继续上述`HTTPS配置`。`证书名称`填写`二级域名`，`公钥证书`填写`cert.pem`的内容，`私钥`填写`privkey.pem`的内容。使用`VSCode`打开这些文件并复制其内容。

![HTTPS设置](./images/6bf2d69fd592e4094ede65c8e41ca757.webp )
...
![基础数据](./images/0465fbe8487a449a419e27e2b6d4fad0.webp )

![用量](./images/ed26f31e217ef10cd54a212f99b384d5.webp )
