...
![](./images/79f43d0160eaad453eecd0f6a13ed584.png )
...
![](./images/4a5db9163f390e1e9d97a16e84785bfc.png )

你会发现 React 在更新 class 组件的时候，会判断如果是 PureComponent，那么会浅比较 props 和 state，如果变了才会渲染。

怎么浅比较的呢？

![](./images/4c3462b6b2547e8a58cc00fb3cc290cb.png )
...
![](./images/28c93642d2a1d56506718e6ef95d0c87.png )
...
![](./images/21ec85b084ba7c54c52747405f7a697e.png )

用上图的方式 setState，整个 state 变了，但是 key 对应的值没有变。

在 PureComponent 的 class 组件里，按照我们的分析应该不会再渲染，只会打印一次 render：

![](./images/6ca77b73a029dfe43c031e78cc2da614.png )

确实是这样，虽然 state 对象变了，但是 key 的值没变，不会重新渲染。

然后在 function 组件里试一下：

![](./images/6dac6c7de62df87eea5c8b6845331690.png )

你会发现它打印了两次 render：

![](./images/6268ff6b5d38ca19ec36afe904e809b7.png )
...
![](./images/bcaf900c6fbda5e6228ca398c180e4f4.png )

state 的每个要修改的 key 的值，如果是个对象，那要创建一个新的对象才行。

function 组件里也是，要这么写：

![](./images/c800ef6f82655eb17ed7d16ac6ec40fd.png )
...
![](./images/7c822a2de5414c872368611d25c9e01d.png )

而你只想修改其中的一部分，要把整个对象复制一次：

![](./images/bcf07b737201e91a79933a381331cb25.png )
...
![](./images/25953cd535b7288d6765f87906b9ebdc.png )

你和之前的 a 属性的值对比下，发现也不一样了：

![](./images/60ccdb04aa826713f406073b445a8cbb.png )
...
![](./images/44232be5664e7a9bac736784b2be61a7.png )

这样修改了任意属性之后，都能拿到最新的对象，这不就完美解决了我们的痛点问题么？

你还可以用 toJS 再把 immutable 数据结构转成 JS 对象：

![](./images/4332d984ed34fc1ce8d958affecd6f12.png )
...
![](./images/d305c16cdf08508342c4c1dcc7abc5a1.png )
...
![](./images/f11fa671b09a70eaa04c906114199f6d.png )

function 组件里就可以这样写了，把整个 state 用 fromJS 变为 immutable 的，然后后面修改用 setIn，获取用 getIn。

![](./images/56950e37ec58093a52beaf1b90e2eeb0.png )
...
![](./images/946d3284e0d4244de9bc7dd55b5c209f.png )

后面就是普通 JS 对象的用法，也不用啥 getIn、setIn 啥的。

我们在 class 组件里用一下：

![](./images/e165d8efe47f8ce34e7e400bb3253ead.png )
...
![](./images/a7cb19a10c4950d53ce4de362ac8fc77.png )
...
![](./images/5063b99b8e4266cba470e56ee7f82c67.png )

immer 这样写：

![](./images/7180c0ee3b62534912f15ac36a8ee011.png )

function 组件里，immutable 这样写：

![](./images/7962c58a066435c190070d88417611bf.png )

immer 这样写：

![](./images/3f3596a03e976b0bc09f55be2c22907d.png )
...
![](./images/c159d5546db42bf6260b184c826bfd37.png )

而 immer 没有自己的数据结构，它只是通过 Proxy 实现了代理，内部自动创建新的对象：

![](./images/50ee0ece60b3ae2be1aa7eeefa0851f4.png )

只不过是把手动创建新对象的过程通过代理给自动化了：

![](./images/838935772fce1aaae557fe5f4e686fb5.png )
