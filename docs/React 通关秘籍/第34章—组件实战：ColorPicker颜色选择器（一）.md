...
![](./images/85c10632a1e375382bee727484bc42a7.png )

antd 也有 ColorPicker 组件：

![](./images/c2ed14861a77c2d0fc4454cdc2c1e8f2.png )

其实浏览器原生也支持 color 类型的 input：
```html
<input type="color"/>：
```
![](./images/f48aa76daf73926082ec2ac97b9e39a1.png )

功能更强大，还支持网页颜色吸取。

兼容性也很不错：

![](./images/c4074662a5e2fc3c9deef9f0d11dea47.png )

那为什么 antd 还在 5.5 版本实现一个 ColorPicker 呢？

![](./images/f2f182717d0ec08fbcbfc52e1aa5a6ac.png )

主要是为了统一 UI，因为浏览器原生组件各个浏览器都不一样。

比如 safari 的 \<input type="color"/> 是这样的：

![](./images/22e0ed50f80fa17f701903cf18f1b093.png )

safari 的这个做的还挺复杂的，还有一个原生的窗口来做选择，支持的功能挺多：

![](./images/7190d50f2ac94e84b041e034708bd1ef.png )
...
![](./images/4717be4da6a0d8d821cbd7d5225782d1.png )
...
![](./images/b25176ac6c6a115cd7b2fe8cea3bd61c.png )
...
![](./images/4cbc0cb894057457a1f8f0206128ad97.png )
...
![](./images/e2e6e1d858dfd01d307de27205fa5d87.png )

所以说，HSL 对人很友好，调解下明暗度、色彩饱和度等很直观。在网页里支持 RGB 和 HSL 这俩表示法。

颜色选择器一般都是基于 HSL 来做的：

![](./images/3c8c4d038eb0b9447b665b626921be9b.png )

你拖动下面的色彩条的时候，调节的就是色相环的位置，色相环为 0 的时候是红色、色相环 360 的时候也是红色，正好转一圈。

![](./images/9b4f6a47f1f1dee7e0b864cd66a5d145.png )
...
![](./images/7dd651128b50ce068c2bb12cc9b550d1.png )
...
![](./images/651e58cb5ad1cf122a62e271d6bad0a3.png )

然后我们具体来看下 ColorPicker 的每一部分怎么实现：

![](./images/9bb20a05d6ac4e67ee0a85b460e8027b.png )

下面这个透明度滑块很容易理解，拖动改变的是透明度，从 0% 到 100%，

![](./images/8691a696f2a9197f8013f68fdd9ae59a.png )

滑块设置一个渐变背景就行。

上面色相的滑块也差不多，取值范围是 0 到 360

![](./images/7c7f58ee59e8d2b06bb7fbdf4b68608e.png )
...
![](./images/ae71582c88347293384e29fa457823d3.png )

取出的值是 0 到 360 的色相值。

最后，上面的调整饱和度亮度的部分又是怎么实现的呢？

![](./images/b5c6cf2da36397a9ab0a39fc083a34d0.png )

其实也很简单，也是加了渐变，把渐变去掉就是纯色了。

![](./images/a8657771bb090339e722910bcc02c3d1.png )
...
![](./images/72b4ba67af3882fc36f8c3474feb0e58.png )

这个东西可不是网页里实现的，这个是原生组件，浏览器底层可以很方便的拿到网页渲染的结果，然后取色。

不过浏览器现在也有这个 api 了，叫做 EyeDropper

![](./images/697fda4cc8572865f657c6130e6ad8e4.png )

用起来很简单：

![](./images/56e1f385b1b057e4186f03a4f4404b81.png )

效果也很好：

![](./images/c45260fd9060b8bbf108aa0504ed8734.png )

但是你看看这个惨不忍睹的兼容性：

![](./images/4460d4d520268905b2f622d159057305.png )
...
![](./images/58744c47e1fb8d9a58b8d5f8a438f90d.png )
