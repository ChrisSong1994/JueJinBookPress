# 46.实现 Chrome DevTools 的 Coverage 功能
...
![](./images/b7a46cf544f2bf50c7f2c9f001d7665b.webp )
...
![](./images/cc2f5fc767a8e92670a56ee2061409b1.webp )

在 sources 面板里可以直接看到哪些代码没执行，比如下面的红色部分就是没有执行的：

![](./images/5dfcaa706e4e3cbe1ce173541e7b7ab7.webp )

这个功能还是很有用的，可以帮助我们分析哪些代码是用不到的，可以进行延后加载或者删掉等优化。

在 More Tools 里开启：

![](./images/d5e7e8acf6bf42d5ccd59c4351611ab1.webp )
...
![](./images/17a76183600fe6247b07610c40cdb744.webp )

事件对象里是这段 js 的 url 和行列号，再就是 scriptId。

然后再看下 CSS.styleSheetAdded 的事件对象：

![](./images/2b24f101ad8b2d0ccb6fcd5a870a1f46.webp )
...
![](./images/bdbcab6c6680a2544a5ac0b200a087b2.webp )

![](./images/0c54758abe2a2ba8c7cd490a998804f3.webp )
...
![](./images/4b763629b6bad774a9f992d77cece98b.webp )

返回的结果显示 scriptId 为 89607.4 的 css 的 50 到 80 个字符的代码执行了。

我们在 cssMap 里看下这个 id 对应的代码：

![](./images/2a78cc8129a7cd4f3bdd3d49b7b699c7.webp )

然后取出 50 到 80 个字符的代码：

![](./images/ca77c6a9e5ecef74d79bcda3d899474a.webp )

也就是说所有 css 里只有这一段代码是生效的：

![](./images/704e9d14e17361c9e4e0b2780394c0f5.webp )

你用 Chrome DevTools 的 Coverage 分析结果也是这样的：

![](./images/56d4aea26d932c1d6a45e994afded0e4.webp )
...
![](./images/665ba9c4abf30415128a0ace8fe7af4e.webp )

因为我们页面上就两个 script 嘛：

![](./images/384f8227ddaaf63f32992d7737cddc18.webp )

第一个 script 有 4 个 functions：

![](./images/a94fff47d220643c20fa020c3ac2eb96.webp )
...
![](./images/53273c607f37f58169eecfb5ea581b00.webp )

minus 函数执行了 0 次：

![](./images/7614448f8fa1036c2e567083aa599958.webp )

第二个 script 的匿名代码块执行了 1 次：

![](./images/499fb53f78a5bac336ec7a4bcb9b8415.webp )

这不就和 Chrome DevTools 的 Coverage 结果对上了么：

![](./images/d21329386348c8422aa432b705cbdb4a.webp )
...
![](./images/b76dbf47806274aa79c81261dd4f5bbe.webp )
