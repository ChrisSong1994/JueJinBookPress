...
![](./images/a4e2a29fd2b6214284401d936e68c057.png )

![](./images/5dee521285626e12aec2a51e4072ca8b.png )
...
```
![](./images/351a9133bb6880c1e2cc85ec80963fb3.png )

点击 debug 面板的 create a launch.json file按钮：

![](./images/acba2b84edd92998f53d8198e4d3bb26.png )
...
![](./images/0912a88ad8ed55324d4fc088f1240fc3.png )

在 AppController 的 getHello 打个断点：

![](./images/febcdd6488d3bece8b2046c3ea488725.png )
...
![](./images/3dc90f777550ae769692d3fb9a4c0980.png )
...
![](./images/439222906ca9fba2d0771b85f2236cf8.png )

![](./images/ba075fdeab24ed090560b32468c9b58c.png )

可以看到它做的事情就是 tsc 编译代码，然后把编译后的文件移动到 node_modules/@nestjs 目录下。

move 的具体实现可以看 tools/gulp/tasks/move.ts 的代码：

![](./images/c93badbc3cd9c8cb04fe9bb03912cbdb.png )

所以，执行 npm run build，你就会在 node_modules/@nestjs 下看到这样的代码：

![](./images/c55fae1b5f0d2a829fdc1c11789875a1.png )

只包含了 js 和 ts，没有 sourcemap：

![](./images/413b6c98100a26e4e52fbbd92d37715f.png )

生成 sourcemap 需要改下 tsc 编译配置，也就是 packages/tsconfig.build.json 文件：

![](./images/381e03beaf46e5f994e46c0f1e839168.png )
...
![](./images/7ba75b45f4da55fc34bc9b879b3d0a99.png )

并且 sourcemap 是内联了源码的：

![](./images/6da69b4d5ed6a04c441172fd618989ba.png )

然后我们跑一下 Nest 的项目，直接跑 samples 目录下的项目即可，这是 Nest 内置的一些案例项目：

![](./images/0d5d62b9485557c1af5675cb67d66f6a.png )
...
![](./images/5636dd3e1e1c6828bb56c041cc90d1d9.png )

![](./images/a50e02bcc583ff093dffc98d120ad79a.png )
...
![](./images/151e20e356f08ddf70397e2ecc59727d.png )
...
![](./images/a7a3926bb6659a4c26a74f73658277ff.png )

然后点击 debug 调试：

![](./images/3e5a0ba9acbd569672fdfd84b74f1356.png )

如果提示端口被占用，你需要先 kill 掉之前的进程再跑：

![](./images/886b7a0b4512446858febaf5f3bb09bb.png )

![](./images/b9afb04efbdc90a6ddcf84fe5adf72f9.png )

浏览器访问 http://localhost:3000/cats

断住之后你看下调用栈：

![](./images/dde4720eb6fe0c2e82442f36039ecc5a.png )
...
![image.png](./images/8db0386888be9ae14b5982b34602ae58.png )

可以看到它先创建了所有的 pipes、interceptors、guards 的实例，然后封装了调用 pipe 和 guard 的函数：

![](./images/ffe2a364186095ac79bda100303c305c.png )

下面调用 hander 的时候，先调用 guard、再调用 interceptor，然后调用 handler，并且 handler 里会先用 pipe 处理参数：

![](./images/0e1bb02696a595e87e924a37694dd6d0.png )

![](./images/0c020024a7998d5542b3bfa4cad25504.png )
