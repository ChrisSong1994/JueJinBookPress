...
![](./images/6e7c40b8b2d2cdab9df8a4a6ff643277.png )
...
![](./images/b57727d3980911510ee3bdee1602f716.png )
...
![](./images/8739364ba278366b6677bc5045a8adf9.png )
...
![](./images/8550629bde83ac7fe5ea5c2f846d73d9.png )
...
```
![](./images/f6cc80c3d8c6fb47439080eed68b95fc.png )
...
![](./images/d42ece52763ea623b006efc8c15484c7.png )
...
![](./images/7d329bc16d63996b0e371ed80c4c292c.png )
...
![](./images/99c9a1e7ab58c49e8443119a10906c41.png )
...
![](./images/1cac1e8b1d260d29eced89b0d7a79781.png )
...
![](./images/4b3ad9c1e04f8a85b48742f2becf332e.png )
...
![](./images/d0ef98b90dfab3cefb8e0f88a83ce643.png )
...
![](./images/3a3ae98d40c89dfb37c9ca92f950f7fc.png )

注入的对象就是连接这个微服务的客户端代理：

![](./images/98dcd895a3faa12720060fc6d67bb7e8.png )

调用它的 send 方法，第一个是消息的名字，第二个是参数。

这里的 sum 就是微服务那边声明的这个消息，而参数就是那边声明的参数:

![](./images/cf33122225bfc89a286baeb285b8e9fe.png )
...
![](./images/e868c496144bcd0c156ca8890507cdc9.png )

http 服务跑起来的提示是这样的：

![](./images/65db06d97c5f228ea075a39996d2df2a.png )

然后浏览器访问下 http://localhost:3000/sum?num=3,5,6：

![](./images/5a92f65ad235adf0e7c2d548b4c02c88.png )

返回了 14，是 3 + 5 + 6 的结果。

浏览器把 3、5、6 的参数传递给 http 服务，然后它给微服务发送消息，把参数带过去，微服务计算后返回了 14 给 http 服务，它再返回给浏览器：

![](./images/6045cdae29f91f4739934bac3f86c08d.png )
...
![](./images/195ff772456a15e929f626319bc5e621.png )

然后在 microservice-test-main 里调用下：

![](./images/d878915fb1297027206f74cf951b3bf3.png )
...
![](./images/18eccbafb52f8857c2964de5567db21c.png )

![](./images/4f8319a2fd2c647726b953aa60d1f3e9.png )
...
![](./images/277ec33a456e2f0495db4db3f5bcbaa5.png )

安装后把它跑起来：

![](./images/5397d219229d7cc6efc4511d843d628a.png )

选择 loopback 这个网卡，本地回环地址，可以抓到 localhost 的包：

![](./images/a1732dc679c3bfcab9edcc5b7ad7b09a.png )

输入过滤器 port 8888，也就是过滤 8888 端口的数据包。

然后回车就会进入抓包界面：

![](./images/22864a4054f61b584911fa5aba34c8da.png )

这时候再访问下 http://localhost:3000/sum?num=1,2,3

![](./images/9b72285162c5f9aae37ed530d9bb2188.png )

可以看到抓到了几个 tcp 的包：

![](./images/6ec4ee7e450a6d04f7549ab2be619fea.png )

点开这几个 PSH 的包看一下：

![](./images/4c7f87e8b6a3f65e5f98af2dbf59330c.png )

![](./images/9b4dc698cd9ef3bdfb008cf9d6c15190.png )

![](./images/2bfa9571d786ad2306dbc30e4109dd46.png )
