...
![](./images/00399dc1fed63aa655d9e7fc2014f515.png )
...
![](./images/cf5aea12981352caf587b198e2f5b8b1.png )

随便找个有 websocket 的网站，比如知乎就有：

![](./images/1d9287e545f5283199cad9bb3e981435.png )

过滤出 ws 类型的请求，看看这几个 header，是不是就是前面说的那些。

这个 Sec-WebSocket-Key 是 wk60yiym2FEwCAMVZE3FgQ==

![](./images/52f5a8806359518c35ba4c999ad66fba.png )

而响应的 Sec-WebSocket-Accept 是 XRfPnS+8xl11QWZherej/dkHPHM=

![](./images/022b08d749d2695c4b5bc79752daf13e.png )

我们算算看：

![](./images/4b1d303999e7b3d487758da7709cc51f.png )
...
![](./images/f0b43ec61d7e4682a332921365c31744.png )

全新的协议？那具体是什么样的协议呢？

这样的：

![](./images/c12721098cebf0777f91970425bb1da3.png )

大家习惯的 http 协议是 key:value 的 header 带个 body 的：

![](./images/62d74f393365396614d7ff49be6cea21.png )
...
![](./images/c4ee37470fca7525eb664caae07029e9.png )
...
![](./images/d64de943aea11fdc282fd6fc67718040.png )
...
![](./images/76a8475961f47883613cc9921f489104.png )
...
![](./images/3b753f9d708b6e02f15a1aa3d6cbcc2b.png )
...
![](./images/9353e3bfa8f1069d318febd1b15d94ae.png )

这 3 个 header 还有 101 状态码都是我们返回的。

message 里也可以看到发送的消息：

![](./images/29c644573ced074e392e82056a021453.png )

再去服务端看看，也收到了这个消息：

![](./images/a9a6c45ef3e8b94073f89d731b2d669c.png )
...
![](./images/17f62c383e5e85fede53e4031969755f.png )

我们需要第一个字节的后四位，也就是 opcode。

这样写：

![](./images/d27e798459075c02f21e60986fd480a2.png )
...
![](./images/33a0690be008da318b1fe9cfc81d0387.png )

第一位是 mask 标志位，后 7 位是 payload 长度。

可以这样取：

![](./images/33e81036d5670ae2388bf6eda9e21e1e.png )
...
![](./images/08855d06621840f07274a19f812b2a63.png )
...
![](./images/e41f65c4839a5deb26c1550a983e8e49.png )
...
![](./images/8071a101524d952b19bdffb76ac027e5.png )

这样就拿到了 payload 的长度，然后再用这个长度去截取内容就好了。

但在读取数据之前，还有个 mask 要处理，这个是用来给内容解密的：

![](./images/61a3ee89c4bb9012b9fd51ae790edcc0.png )

读 4 个字节，就是 mask key。

再后面的就可以根据 payload 长度读出来。

![](./images/07476538f8be165ed77c86aab2f044ca.png )
```javascript
let realData = null;
...
![](./images/d5f97400cef6a7e484753f778b7df728.png )
...
![](./images/7de1113cf3afc93b4b2d849b58d76a1a.png )

而现在我们能正确解析出其中的数据：

![](./images/9f5db0e1319026d12a3a7af54d0fb483.png )

至此，我们 websocket 协议的解析成功了！

这样的协议格式的数据叫做 frame，也就是帧：

![](./images/6762e0ee0985efd2e99ed794764ace47.png )
...
![](./images/ace2d9c99231db8842f96c3e00dfc44d.png )
```javascript
send(data) {
    let opcode;
    let buffer;
...
![](./images/c28462e198b910319522f5226da79465.png )
```javascript
...
![](./images/f559c8d72eb912db0bb7fd498e552b80.png )
...
![](./images/c204955a47fb40053a678108a8fe8b13.png )

收到客户端消息后，每两秒回一个消息。


![](./images/a463c936eb879bc6835574215b172513.png )
