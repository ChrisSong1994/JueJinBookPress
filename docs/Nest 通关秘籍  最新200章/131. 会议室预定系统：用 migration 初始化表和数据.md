...
![](./images/2e87df20dc5fa82619ebc460efa10c28.webp )

![](./images/dfd737c4b0ba90b5d0a6a0bc9762303d.webp )

这样每个表一个 sql 文件：

![](./images/b5330e6d42c329ca972db7915cbbdefc.webp )

我们选择第二个选项，重新导出下：

![](./images/915de686ee5b8fc55b30f55916514293.webp )

![](./images/9eea8579e60c70e5492f47ca31f0c959.webp )

这样就都集中到一个 sql 文件里了。

打开这个生成的 sql 文件看下：

![](./images/42a62748543f6017a9dc67a9f10d6219.webp )

包含了所有的表的 create table 和 insert into 语句。

然后我们在项目的 package.json 里加一下 migration 的 npm scripts：

![](./images/4c02c53ecca7ea36b45a999594ef2137.webp )
...
![](./images/35e33feb401b89145c756d31bc0e9e29.webp )
...
![](./images/c71e0ca94511acbb8a285ff35096fe2a.webp )
...
![](./images/aa4a2ad89630cf60b7714a5468c2d573.webp )

![](./images/7c43609f4852386fd29d0bcd5103d5e4.webp )

这时候没什么更改：

![](./images/fc5e6e2ec5a1987a17ac9a8e24de7459.webp )

因为 entity 和数据库表是同步的。

我们把数据表给删掉：

![](./images/c46936c9fa5507012e62847f56768946.webp )
...
![](./images/adcd8a3e613f1b0a70c49a7eb5cc8866.webp )

里面包含的自然就是所有表的 create table 语句：

![](./images/dc27e4fa73ee7b61dc56be5bbe410061.webp )
...
![](./images/0c9ffe83da1a99383bb075767e37ff5c.webp )

数据库里也有了这些表：

![](./images/da0d0138de7555e343e374b05a66b225.webp )

并且在 migrations 表里记录了这个 migration 的执行记录：

![](./images/5776a66543987d52ca31fd5eabc582f2.webp )
...
![](./images/2b7958ebc5612be0f8814ca63a5caa81.webp )

这次我们自己来填 sql：

![](./images/9b44ca0d510d965c06f8fa2f0685b899.webp )
...
![](./images/076346e7c2b11c64ae97486dfc5872ff.webp )

也就是列的顺序对不上。

因为 sql 文件里的顺序是这样的：

![](./images/495b875d301b9cd7fe13f77d2673bcca.webp )

第 6、7 列是 userId、roomId，和 insert into 语句对应。

而现在第 6 列是 createTime：

![](./images/b50f87d8e4b77b4df546d82fc43ecfac.webp )
...
![](./images/224ca0cf66c505bff1720250f0e97ef1.webp )

在输入框粘贴下：

![](./images/7e5e21bb2c674f3440975a404eda37be.webp )

把列名这部分复制出来，调整成正确的顺序然后放到 insert into 的 sql 里：

![](./images/e50b6b3cd81596c6c8894c684ef05e1e.webp )

只有这一个有问题的 insert 语句指定下列名就行，别的 insert 语句不用改。

然后你还可能遇到外键约束的问题：

![](./images/60ce5e04bf257460ff13c68645d31760.webp )
...
![](./images/02eb4e1b2f0f050e456284b27f6c57b3.webp )

执行成功了！

在数据库看下：

![](./images/8bbe8331a40c607a2db710b1b12857a1.webp )

![](./images/53c961276a2c59ae8fdc394ace7c8e58.webp )

数据都插入成功了，并且 migrations 表也记录了这次的执行记录：

![](./images/1d7138037d2652510af0a5be4d95f3e5.webp )
