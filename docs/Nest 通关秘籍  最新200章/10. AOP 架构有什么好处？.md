...
![](./images/f01ce5337e1416bd0fc4ee4df404bc5c.png )
...
![](./images/f8f7b368b924bc096733c3f5c693efc4.png )
...
![](./images/47aeba20ece092c12fe099fef4fb3c8a.png )
...
![](./images/83bdbe474248def201fa77b21994c0f9.png )
...
npm run start:dev
```
![](./images/6aa2061e70e6d858228eecf78f787af0.png )

浏览器访问下：

![](./images/4ecb207e6e8abea5c43329a4f616ad83.png )

![](./images/87cd0479b750378acec2593375f82976.png )

可以看到，在调用 handler 前后，执行了中间件的逻辑。

我们再添加几个路由：

![](./images/dc90439451c4c0cc3a896e635f81e670.png )
...
![](./images/dd3dcfcfccf905fdb04fffa1e69c7f35.png )

![](./images/7a9155d8b1dbb8232ea010e608028661.png )

可以看到，中间件逻辑都执行了：

![](./images/27f59d3ae2baea11053a713ff9b299bb.png )


也就是说，可以在多个 handler 之间复用中间件的逻辑：

![](./images/90ea8049fbb0366127920d5d95600a5b.png )
...
![](./images/963d5b007bb9f2556bf994f313de88dd.png )

--no-spec 是不生成测试文件，--flat 是平铺，不生成目录。

生成的代码是这样的：

![](./images/d76f7488bdfc42a0f7b84cb563accfff.png )
...
![](./images/47374f6d941414aaa1273f83cc12ceca.png )
...
![](./images/ffa125e5b0fac0858cda7e6b0220d7ad.png )

![](./images/f66f0d8061348c59b2909c95cc473f4c.png )

![](./images/01473540874aff66e2f2b8cc0fcbae39.png )
...
![](./images/53cecd0306b40c0aa3a5bfdcd6b45166.png )
...
![](./images/4744d22527a3bcd86c8651702a8bce17.png )

生成的 Guard 代码是这样的：

![](./images/dea15f7e19dce7ce650d963208b59700.png )
...
![](./images/78f100b40548620ed69205418c7dea19.png )

然后再访问下：

![](./images/42414c352318462610bdd3b7b2a7f54b.png )

![](./images/c9837e4310d7f3384c67d5f7d11db3f8.png )

aaa 没有权限，返回了 403。

![](./images/20d75bbb6ed751abe457dfd760976743.png )

Controller 本身不需要做啥修改，却透明的加上了权限判断的逻辑，这就是 AOP 架构的好处。

而且，就像 Middleware 支持全局级别和路由级别一样，Guard 也可以全局启用：

![](./images/353518289758737dea8965bb7e678fa8.png )

这样每个路由都会应用这个 Guard：

![](./images/289dd431ebd4f9b5e8ebb9061e34a241.png )

还有一种全局启用的方式，是在 AppModule 里这样声明：

![](./images/42e462c661068384f278216863ff7da0.png )
...
![](./images/3c5faacc43541edb67b11b1f586402b5.png )

再试下：

![](./images/d1c5114069edee8ae2a0ec61a92a7e82.png )
...
![](./images/425afb494da7bdf7659a867901df138a.png )

而用 provider 的方式声明的 Guard 是在 IoC 容器里的，可以注入别的 provider：

![](./images/f309d02e01a5829c802e1c93f8945d35.png )

我们注入下 AppService 试试：

![](./images/2be7b21513b7d9f2a1ddb8f50159bf5d.png )
...
![](./images/de4b7ec5efac5282b3ba49e222e061ee.png )
...
![](./images/cb992ad442b64944b06d4c83af8af3bd.png )
...
![](./images/ce99948d3478239010d51ac0ac9aa8b1.png )

生成的 interceptor 是这样的：

![](./images/80c9149324769bcbcac6dd00613ffa4e.png )
...
![](./images/a6dd920be1e563c162a4211cd3ea8adc.png )

然后启用这个 interceptor：

![](./images/78e702488bd8eb3b43c49d4958af27cd.png )

跑一下：

![](./images/8579a53436a78d79911837e89fab1876.png )
...
![](./images/54edb24975550cf91610d99224879f70.png )

![](./images/cb3a0bd70778177556686376613fa678.png )
...
![](./images/5dce3138a827a1d75c288b67c70ceac6.png )

也可以在 controller 级别启动，作用于下面的全部 handler：

![](./images/ca62b213c69cddd5b7a8e03cf0725381.png )

也同样支持全局启用，作用于全部 controller：

![](./images/da9a87b4d9666498eeb82985cc163366.png )

![](./images/f0a9a6aa63157edba32d0eb1629768fc.png )
...
![](./images/5dcb9e78f1f046bec106c4e71c0e5c88.png )
...
![](./images/1f98e60adae8403bbc14ed3843f4c80c.png )

生成的代码是这样的：

![](./images/dc399f07c3c3dcca61930dd3b7afbe76.png )
...
![](./images/62db3316e6bb847a4ea70e0b07f3f3d9.png )
...
![](./images/2bea70455ab8815911d5537b75088b08.png )

![](./images/9e32de6810209252134e6832afbd9eca.png )
...
![](./images/63d437d1b1b4f4b1fee9c09c914e8844.png )

![](./images/29675032c3cccd1f99b2141e738253fd.png )


或者全局生效：

![](./images/fb108a6fd291eb0f9117e2e2ca5d79f8.png )

![](./images/a6f1d033f64fa2535e06065ca97d249b.png )
...
![](./images/631c1b3631817650a8fd0f40aac3de65.png )

其实我们刚刚在 pipe 里抛的这个错误，能够返回 400 的响应，就是 Exception Filter 做的：

![](./images/c256808d72370876e5914d7e5af5b7f9.png )
...
![](./images/3ec58ce422aecc92b510e525ed745d1f.png )

生成的代码是这样的：

![](./images/3a4fff17036f2969ff47574fa9c3fc90.png )
...
![](./images/c76e43d6480d0eb4c4f0dd723afd160b.png )

再次访问，异常返回的响应就变了：

![](./images/8927f8568f139358219cb16455cfbb46.png )
...
![](./images/4f3f0b8b1edb0203336f19e32eec1ef0.png )
...
![](./images/3c83b00d0634aee384aaa2ee67d9d542.png )

全局：
![](./images/1a658771a9441ed39d1d72af74a2b3cd.png )

![](./images/5c15269c3aa9472ddb2c822888046d11.png )

![](./images/cfd89c86c522d1741f02e2f09dcef225.png )
...
![](./images/1cf26a73320db7a448fbf4d3276bb76a.png )

很明显，进入这个路由的时候，会先调用 Guard，判断是否有权限等，如果没有权限，这里就抛异常了：

![](./images/8c15aa9dbdbcd50987fe415e8353a16d.png )

抛出的 ForbiddenException 会被 ExceptionFilter 处理，返回 403 状态码。

如果有权限，就会调用到拦截器，拦截器组织了一个链条，一个个的调用，最后会调用的 controller 的方法：

![](./images/de1bc1f6777331ead505175be551ced5.png )

调用 controller 方法之前，会使用 pipe 对参数做处理：

![](./images/2e7452c56d9a228d357f24d2cdbe0ce7.png )

会对每个参数做转换：

![](./images/9799a057efb516e14fc650ec18d047b5.png )
