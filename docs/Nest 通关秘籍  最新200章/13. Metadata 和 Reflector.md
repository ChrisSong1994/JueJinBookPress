...
![](./images/942a082b5434b509b05fbec5ee9ac7e0.png )

Reflect.set 是设置对象属性值

![](./images/8d9a71b8c29c8b5f77a08ebdc50bed8d.png )

Reflect.has 是判断对象属性是否存在

![](./images/481b9fd34566cc5ba7440f3584064dad.png )

Reflect.apply 是调用某个方法，传入对象和参数

![](./images/c047597b5471388ac1784baf84472096.png )

Reflect.construct 是用构造器创建对象实例，传入构造器参数

![](./images/8f736cc692883e4e530adf2055609908.png )
...
![](./images/23f3acdd8986c48c2998cf8d16ee1bd9.png )
...
![](./images/184594c7d821e45e82f068e6d17d05b0.png )
...
![](./images/71a5483f8d74971e11ecd89fbac645b0.png )

![](./images/763eb807799e001d01bd0e0dee97e893.png )

而且 @Controller 和 @Injectable 的装饰器也是这样实现的：

![](./images/83b8f73b5e9f8c816ec3c97cee1a1eb5.png )

![](./images/ea19e421c7e2a5f74ba8942bd36b51f3.png )
...
![](./images/a66e087b1a1ef226cac1f8406470913c.png )

但是呢，ts 有一个编译选项叫做 emitDecoratorMetadata，开启它就会自动添加一些元数据。

开启之后再试一下：

![](./images/10b0d465698007fe7e7f6e72f7d4a40e.png )
...
![](./images/351dd2026e66df37033d6d812f640698.png )

就是获取构造器的参数类型的。这个常量就是我们上面说的那个：

![](./images/6c507a94c8d8bcc250bd05c9783873ca.png )

这也是为什么 nest 会用 ts 来写，因为它很依赖这个 emitDecoratorMetadata 的编译选项。

你用 cli 生成的代码模版里也都默认开启了这个编译选项：

![](./images/e3dfa6b612b35c90de85d5c052cb1bfd.png )
...
![](./images/ea1b43eafee47189ec11968da815a7ea.png )

这个装饰器的底层实现自然是 Reflect.defineMetadata：

![](./images/10cd0ceeb5e0e72a5c1f615cd9e53375.png )
...
![](./images/d6a63d5bb6be4e5d3b94925f0e01dab5.png )

在路由级别应用：

![](./images/5ce5ebf7c3157ea0e2cc5238513540ed.png )

加个打印语句：

![](./images/3699e47dd19f6fb5055e2bcbb9cb0c5f.png )

![](./images/bc2b2a06748e77e3f460e5213c9cdf84.png )

然后 nest start --watch 把服务跑起来。

浏览器访问：

![](./images/26638cc75bd169ddefef2f82c7dd7097.png )

可以看到 guard 和 interceptor 成功执行了：

![](./images/fd981fb3f23f971ab7720e083b1ebcac.png )

然后我们用 @SetMetadata 在 controller 上加个 metadata：

![](./images/9d60a40899099acb568ae8075c18d26c.png )

在 guard 和 interceptor 就就可以这样取出来：

![](./images/9b6bc24bb5d30ecfc82492910909db47.png )

通过 ExecutationContext 取到目标 handler，然后注入 reflector，通过 reflector.get 取出 handler 上的 metadata。

interceptor 里也是这样，这里换种属性注入方式：

![](./images/52c2e44ab45d1f7957eff14961283ea5.png )

刷新下页面，就可以看到已经拿到了 metadata：

![](./images/8779fd083e51089a6337ef4718ce0e9a.png )
...
![](./images/098523c555365eb7a0ce5e3e71a063e9.png )

![](./images/5b5bde9fc79119c032251b5ed4028790.png )

![](./images/062cd6e038644e2e1e5573515d122cdf.png )

reflector 还有 3 个方法：

![](./images/bd8a1534864bf1b73d62fc8a22cfa956.png )
...
![](./images/d1126d4e84df02001c6bf0996b25fb68.png )

get 的实现就是 Reflect.getMetadata。

![](./images/c43de7980cb4b181e1fc8e8fc1a0eed7.png )

getAll 是返回一个 metadata 的数组。

![](./images/3d9ab2d4bf6e5ef3b131b05a8be5c1f8.png )

getAllAndMerge，会把它们合并为一个对象或者数组。

![](./images/b93331574be4c474bc7408b43e04d5bd.png )

getAllAndOverride 会返回第一个非空的 metadata。

我们试一下：

![](./images/d7bad32c2b8d3db9d733222d9e9fd6f2.png )

可以看到它们结果的区别：

![](./images/5d681d0d49c4d0df2c4d8b5527419ff6.png )
