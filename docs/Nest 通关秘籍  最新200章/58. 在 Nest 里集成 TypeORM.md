...
```
![](./images/c61c3498143cf8b3db2933e0f12bade1.png )

然后创建一个 crud 的模块：
```bash
nest g resource user
```
![](./images/a188ce83fccb0378e004e1d8a3234f4d.png )

生成的 service 里的 crud 并没有真正实现：

![](./images/d97da45e9955f11d81433b0df2007416.png )
...
![](./images/6946534a24ab049525e2ec5158ea1860.png )
...
![](./images/79c4a1bbfefc79047cfd2d733a237935.png )

这部分和我们单独跑 typeorm 没啥区别：

![](./images/19c1fec5c80f18f3a945b7a145445566.png )

然后是增删改查，我们可以注入 EntityManager：

![](./images/8f2427881c79b4a8d7e958338941363c.png )
...
![](./images/5549887d6265009f5465b3599865f506.png )

服务端打印了 insert 的 sql 语句：

![](./images/be9dc971d4537195b09873ba9ed47a8a.png )

表里也可以看到这条数据了：

![](./images/4923114bcf3e66c47a2fd57d3ba3bfe0.png )

对应的是这个 handler：

![](./images/1ad8035839ef13fb2d07bbf256c4a0fd.png )

然后再试下查询：

![](./images/68e0e908087f8a33ce58b0fb5a7971dc.png )

![](./images/de934ecde74dca11b36a9fe99588f7f0.png )

![](./images/8b3c8ee5eb92226bad8c7a6ba51040d7.png )

单个查询和全部查询都是可以的。

再就是修改：

![](./images/04d23b7dab0328486fdf53ccf458168a.png )

在 controller 里是接受 patch 的请求。


在 postman 里发一下：
![](./images/923afffb39842ce937b0beaf90dd15ff.png )



可以看到生成了 update 的 sql 语句：

![](./images/48c1e540ce95eb632177881f9e1182c2.png )

数据库中的数据也被修改了：

![](./images/ca12854c68da05d6c53fd9cdb11817fc.png )

再试试删除：

![](./images/f5ccd2239b796efbe3552b35d9a75305.png )

在 postman 里发送 delete 的请求：

![](./images/993099bd21b0d7223637cea8360d70fd.png )

可以看到生成了 delete 的 sql 语句：

![](./images/38d74f8362a0da405d869768a70ffb58.png )

数据库里的数据确实被删除了：

![](./images/aec384ce387f3b7f341c6e09b4f072a1.png )
...
![](./images/75dba437236c7d71f6029bf835d98eeb.png )

它会引入 TypeOrmCoreModule.forRoot 的动态模块：

![](./images/fa2acfa98b90fc4e782801fa3a590a48.png )
...
![](./images/74fc117ca7d7649f51f8f276d9e74e03.png )

而创建 entityManager，则是注入 dataSource 取 manager 属性就好了：

![](./images/aea7a7c67261b8ba3b58bc2913e8d1ff.png )

然后 TypeOrmModule.forFeature 则是通过全局的 dataSource.getRepository 拿到参数对应的 Repository 对象，作为模块内的 provider。

![](./images/451ebfdc3ff1bb1e1110cfa74d625408.png )

![](./images/bc04b109a86ddd28a5ec6ebf2d2c0d57.png )
