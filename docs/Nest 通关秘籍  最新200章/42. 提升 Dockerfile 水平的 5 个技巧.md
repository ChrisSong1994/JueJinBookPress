...
![](./images/4abb0c4565441cf4e396ebad2dd9f140.png )

但是，这个容器和宿主机还是有关联的，比如可以把宿主机的端口映射到容器内的端口、宿主机某个目录挂载到容器内的目录。

![](./images/304eec324561cc77fb5bfe41e0c6e022.png )
...
![](./images/711148e8cea7e39e9a45282be9ccb46e.png )
...
```
![](./images/6d918258609f623ad5a4c6a509dff052.png )
...
node ./dist/main.js
```
![](./images/4ba4eadd03fa7b154bf50adfd13da7f6.png )

这时候访问 http://localhost:3000 可以看到 hello world，说明服务跑成功了:

![](./images/07fbf36e7bd0c1d046a34d2caee81ebc.png )
...
![](./images/82a1bed3b328caf4a603b3413ff68574.png )

然后在 docker desktop 的 images 里就可以看到这个镜像了：

![](./images/a64bc89858d453cf8891c2804a0397ba.png )
...
![](./images/00718fb1412e61b8a38bda21e5caeabc.png )

然后就可以看到容器部分有了这个容器了：

![](./images/5e040a8908ba5f28d7b23caf5fddc0be.png )

浏览器访问 http://localhost:2333 就可以访问容器内跑的这个服务：

![](./images/37cad6a0bf3edead9309ed32dc4d280f.png )

这就是 Dockerfile 构建成镜像，然后通过容器跑起来的流程。

![](./images/989c408d1c09eadbd515a618d1e65489.png )
...
![](./images/d1667103fa5409a87b43c30247b03667.png )
...
![](./images/276455b89922eb81c2d9a6eddb712901.png )
...
```
![](./images/f1381b91c1f3d5b681208613a8370809.png )

这次的 tag 为 second。

然后在 docker desktop 里看下：

![](./images/8e8015dd481eae8516eee3c061de7b18.png )

好家伙，足足小了 900M。

我们点开看看：

![](./images/5f513d58dab5415b9a5c7688778448b4.png )
...
![](./images/1d19b2f7e116dd6411e966eba850c8e2.png )

docker desktop 可以看到这个跑起来的容器：

![](./images/7364381eb87878f0b0f672d58dc69b63.png )

浏览器访问下，依然是正常的：

![](./images/ea0f1910b57dc42c7acad024ce17dc87.png )
...
![](./images/e856722b0b1a60ba296ad3efb0b997d1.png )
...
docker build -t dockerfile-test:second .
```
![](./images/167cf4b7f03ca6961b61829c620f856d.png )

现在我们改下 README.md：

![](./images/519b9f6c703409bc9dd4bf16b2afc430.png )

然后重新跑 build：

![](./images/58fcfb7edb3a3b4378645a44aa54f5b7.png )

现在花了 25s，其实是没有重新 npm install 的。

然后改下 package.json：

![](./images/1e7910e305b2f3328a4ee66208d1f9eb.png )

再跑 docker build

![](./images/07bb590b3d75c869e846a9ec8b3e0679.png )

时间明显多了很多，过程中你可以看到在 npm install 那层停留了很长时间。

这就是为什么要这样写：

![](./images/b50786f1ee649bb29549ad8f1811fb18.png )
...
![](./images/c1c1cb57c12d4982df74fbb17aa589c9.png )

然后 desktop 里看下构建出来的镜像：

![](./images/81b25c864fd7be6d90c7daa96a6b8188.png )

镜像体积比没有用多阶段构建的时候小了 250 M。

然后跑起来试试看：

![](./images/98d04597d30abd2b2863f7b168522638.png )

这次映射 2335 端口到容器内的 3000 端口。

依然能正常访问：

![](./images/59626c63cc5fe8e01fc7d5b7acaf49d2.png )
...
![](./images/15a79be91e9363fb66015e5cf3c3898d.png )
...
![](./images/23d47e00412d64e4925128b74051cb7d.png )


点击查看镜像详情，可以看到 ARG 已经被替换为具体的值了：


![](./images/25fa88d1b2067ddd58ffe407efc27c77.png )

![](./images/c1cfa947d0803ae00180878c3047c923.png )
...
![](./images/128f7132f4f2eef3a4e12e5147ad4dba.png )
...
![](./images/599214af24b921649c21f6ad2ae18c56.png )

其实还可以写成 ENTRYPOINT：

![](./images/350c45917eaf60c8c6039829ee87ff49.png )
...
```
![](./images/eeff159a233ae6efc5720a0374fd953c.png )
...
![](./images/9e8090586ce06057d333cc597143d671.png )

没有指定 --name 时，会生成一个随机容器名。

就是这种：

![](./images/73106177f32e2e12cefece4264d94100.png )
...
```
![](./images/7adc2c1c91adc3cce40146ad4e47b566.png )
...
docker build -t cmd-test -f 444.Dockerfile .
```
![](./images/27bafb31aa75e5ca8d9674637ada70ad.png )
docker run: 
...
![](./images/b4fa6dfe9a43b48d0d39d45afb0b3aef.png )
...
![](./images/ff791d82a93c1cc3bf48b7d6d5427eaa.png )
...
```
![](./images/314237eff14e7d1480736a7afef38932.png )
...
![](./images/60a78d9564ac8ba602c606352f5018b9.png )
...
```
![](./images/979ded84248803d1d5dfc553740fb391.png )
...
![](./images/6da463951209200c4cc7cf58dc744107.png )
...
![](./images/c29dbd66442bbacc80a4c16ac19cbbb8.png )

可以看到，ADD 把 tar.gz 给解压然后复制到容器内了。

![](./images/6e219de4867dc38aaa8b4f5ec4d7c49c.png )

而 COPY 没有解压，它把文件整个复制过去了：

![](./images/8b1614007e7fb9a9f2ab825cfeebbe74.png )

![](./images/83ffcfc6639b7e714331e6549eb6ab15.png )
...
![](./images/f3ea4ad5676ae01dc6fa657da8e581ca.png )
