...
![](./images/a8beb6dafd8136e9839bfbf3f3296035.png )
...
![](./images/dfa5645b79e6726f667a35824417a6d3.png )

会打印这样的日志：

![](./images/9a0b4094ee7198062fc337564bf9ee4e.png )

这里的 verbose、debug、log、warn、error 就是日志级别，而 \[] 中的是 context，也就是当前所在的上下文，最后是日志的内容。

![](./images/e3a74554a874935636c6fe8d1c249092.png )

这个日志是受 Nest 控制的，可以在创建应用的时候指定是否开启：

![](./images/63a4a1f625f2c2f5eb289bb435d7ce8d.png )

设置 logger 为 false 之后就没有日志了。

你也可以自己决定输出什么级别的日志：

![](./images/7c5d4d4a9d4024674f3a0e5040428b57.png )

此外，你还可以自定义日志打印的方式，定义一个实现 LoggerService 接口的类：

![](./images/607e6ce0a4caea8115d52630fbe81b4e.png )
...
![](./images/5c747cc49f620e6fc7e81b47c5b7ad1e.png )

然后现在项目启动就是这样了：

![](./images/78afa344a9d767e288fc8468c120e10e.png )

刷新页面打印的日志是这样的：

![](./images/753bfb5889f7f5226d3e4c688a7fc971.png )
...
![](./images/934e1ec24f94fc7d12af6adb7a1a4352.png )

这样你没重写的方法就是原来的：

![](./images/59afe2478c309cb2d9d9b114237ef27b.png )

这就是创建应用时 logger 的 3 种取值：

![](./images/f0fffae3b5a8161226f9703db82782da.png )
...
![](./images/53f36865b2a76d90a8a6d08c206ac5ed.png )
...
![](./images/6c66e904e3c66a32ea32a859c8f3f044.png )

通过 @Inject 注入 AppService，并在 log 的时候调用。

现在的日志是这样的：

![](./images/76f2ed2a5ab83e9fb7db6c5014afce47.png )
...
![](./images/f5f6ae98f265bb9741f4006ebfd6657a.png )
...
![](./images/cba9c4e144a99348f19959cda7c41d61.png )

在这个模块里可以直接注入 MyLogger，不用 imports 那个 Module。

![](./images/61a12bfe9a76bfa2a9a8074c46915e55.png )

测试下：

![](./images/9c37b31d6c94bcf0223b1c0835cd1917.png )

![](./images/c989a86cc9cd6b0cab6a9483928a8610.png )
...
![](./images/3c93860b590770a0ea29573d532062a3.png )


![](./images/0e55fc63704f1cbd13db332228977676.png )

每次 imports 的时候传入不同的配置：

![](./images/fb0e137df79ca432fc32bd92af229729.png )
...
![](./images/09ab87b44f18f4017a2686bab400b963.png )
