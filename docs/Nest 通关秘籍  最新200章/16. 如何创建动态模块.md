...
```
![](./images/8128db96fff8e628680eda77bc4f08f0.png )
...
![](./images/99fade18ce521f5b3feb70fdb7fa7b48.png )
...
![](./images/74868c498027e1f654ef25c98c29cf65.png )

这个模块是静态的，也就是它的内容是固定不变的，每次 import 都是一样：

![](./images/4eca202c63c672e3fb1b0e619a295b28.png )

![](./images/a1e01eba7de8a85a432fda0b00ebeaa6.png )
...
![](./images/bc211eda0be0faccc4ecb68bb0500768.png )

![](./images/b1388deefb3a59ffd1a192dc40e0572a.png )

而且我们还可以把参数传入的 options 对象作为一个新的 provider。

import 的时候就得这样用了，通过 register 方法传入参数，返回值就是模块定义：

![](./images/ff6acad0d5d1c78677acf54ffd248eaa.png )

改成这样之后，再跑一下：

![](./images/79396dd8785759e8dfc67fe777c9a6a1.png )

依然是正常的。

而且这时候我们把传入的 options 通过 useValue 创建了个 provider，这样模块内部就可以注入它了。

![](./images/6ab4460d83ca41537a15b4e26476b5e8.png )

我在 BbbController 里面通过 token 注入这个 provider，打印下它的值。

改一下 register 的参数：

![](./images/0630582d01d1c7e22c8c1d1ccb7a6061.png )

浏览器再访问下，可以看到控制台打印了 config 对象：

![](./images/a5ab1d4937cf62a2e5043961b834e892.png )
...
![](./images/e313e2728fd11ea7a3868db7d74ac8b4.png )

forRoot 传入配置，动态产生 provider 和 exports，返回模块定义。

而且还有 forRootAsync：

![](./images/77df22d83ae99c8ea06bba4ccb3d5e50.png )

区别就是可以用 async 的 useFactory 动态产生 provider，比如异步请求别的服务拿到配置返回，作为 options。

forFeature 则是传入局部的一些配置，来动态产生局部用的模块：

![](./images/e23bb62442e54d9e1f4b5b25b426a2e3.png )

typeorm 的模块用起来是这样的：

![](./images/ed891103d4ad9d4f2e2d47ef4514ef6e.png )

![](./images/757795fc62f530c8ba25570fb17a728c.png )
...
![](./images/3926fe111733b9a7bbf8b34be4d9e204.png )

这样这个 CccModule 就已经有了 register 和 registerAsync 方法了：

![](./images/25a0b481c8c4107fa7820a10311f99d5.png )

不用自己定义了，省事了不少。

传入 options：

![](./images/32573b1d4b3ac37ed9eae00c436d0e78.png )

那现在如何在 Module 内注入这个 options 呢？

记得 build class 的时候返回了一个 token 么？

![](./images/bbcf847f907a17bb83346d2c38d5e239.png )

就用这个注入：

![](./images/adf24dcf8a6647cc499b2328584f5845.png )
...
![](./images/f0ffd1e699b617a8734caac97e42cfaa.png )
...
![](./images/4f11bcf4a2bfe72d2afd2bb5449b56b2.png )
...
![](./images/d969f5f98881adf38932a2306104560f.png )

![](./images/b26302cb85326b766a6028edabc7803c.png )
...
![](./images/52c56600e89154b5f2c7d409940e20cb.png )

这样创建的就是全局的模块。

不过这样还有个问题：

![](./images/fd58826608140db11c0da98579e8b03b.png )

options 那里多了 isGlobal 属性，但是类型定义这里还没有呀。

因为我们用的是这个类型：

![](./images/2a297de76a3481817d0eed44e56e23e4.png )

最好是用 builder 返回的类型：

![](./images/305fb1f7b68c42361014f501ce55c9f2.png )

这样就有了：

![](./images/030ea32cc5284278c444ee6aea127c9a.png )

而这个 ASYNC\_OPTIONS\_TYPE 是 async 方式创建模块的 otpion 类型：

![](./images/5360b838979fd8d24597176b1c827952.png )
