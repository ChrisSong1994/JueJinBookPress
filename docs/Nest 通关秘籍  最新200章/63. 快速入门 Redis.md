...
![](./images/ab7b6f3eaf27275f46b8f56bd9a094bc.png )
...
![](./images/a47c2db836d0a22344766513a8ce8c18.png )

它会让你填一些容器的信息：

![](./images/44e63848dc9b3c46c19757751bee82cc.png )
...
![](./images/cd6fd97e06b2617313ea29415d61f594.png )

容器内打印的日志说明 redis 服务跑起来了。

files 里可以看到所有的容器内的文件：

![](./images/c5ecba1b424225926e149f86c12c80c0.png )
...
![](./images/9be7790fa16e48372ed319708366e677.png )

在容器内的 data 目录就能访问到这个文件了：

![](./images/0baf81d10d4b746f4a230f1892c933bc.png )

同样，在容器内修改了 data 目录，那本机目录下也会修改。

redis 服务跑起来之后，我们用 redis-cli 操作下。

![](./images/e330df3356224a759610b1b3d6a4846c.png )

在 terminal 输入 redis-cli，进入交互模式：

![](./images/25c67864ab6c2b50d3979997051d8e8e.png )

我们在这里做下 string 相关的操作：

[文档](https://redis.io/docs/data-types/strings/)里的命令有这么几个：

![](./images/05f63dab036968c0b845481475d909ac.png )

set、get 都挺简单：

![](./images/b9d15fc0044a5e636d57a23d9310134b.png )

incr 是用于递增的：

![](./images/51f99428aeb0c50d312f7949712cf384.png )

平时我们用的阅读量、点赞量等都是通过这个来计数的。

当我存了几个 key 后，可以通过 keys 来查询有哪些 key:

![](./images/c59a962037cb6642a191468e8538128c.png )
...
![](./images/395263a5b2801929268b123cdbf60d90.png )

输入操作系统信息，还有邮箱、姓名、职业、手机号等信息，就可以下载安装包了。

![](./images/d6e7ca91a26085fb39413abcad4694c4.png )

安装后就是这个东西：

![](./images/08d78b2915b6869339e3f09f590fd71c.png )

点击 add database：

![](./images/820fb878271bc3eebf2d8ef079b85a4b.png )

连接信息用默认的就行：

![](./images/f0b0b69dd6c1b06b6de1df4a39cc2a1a.png )

然后就可以看到新建的这个链接：

![](./images/81d9e7d3f72bf5e2b5cf25992fee8e9b.png )

点击它就可以可视化看到所有的 key 和值：

![](./images/5fb38cb28638ad59a5f4a01179301ba7.png )

同样也可以执行命令：

![](./images/4c7e142db2d93f9627fed1667dfedd52.png )

然后我们继续看 list 类型的数据结构：

文档中有这么几个命令：

![](./images/a8e7a4803e66eecf5a5d51a9ba3312e5.png )
...
![](./images/b4f787509f258190d03303eaccb17c5a.png )

然后回到浏览页面，点击刷新，就可以看到新的 key 和它的值：

![](./images/a3dd7a2e2a409a926f1f9f25aa11f9d4.png )

这就是一个列表的结构。

lpush 是 left push 的意思，执行后会从左到右添加到列表中。

![](./images/13de4ebc6a543ec62f03001c1429a47c.png )
...
![](./images/0201f298fcb796cdab09eb7f2ae89f32.png )

![](./images/352ef34bf013e0354590ee23cf123f27.png )

![](./images/5d7bc6adb74b12580ea4cdb35db2c011.png )

lpop 和 rpop 自然是从左边和从右边删除数据。

    lpop list1

![](./images/5334699f7e79d5d1073ad789f4f869ed.png )

![](./images/b224f8544f8c9c9e0d74ea34e4cba06f.png )

    rpop list1

![](./images/d891e6fd5c0e90e8ca8c072b78dd909b.png )

![](./images/6bf94d998f241c28556fc681b050fb99.png )

![](./images/53118ec4efeeca278b60952eea75541c.png )
...
![](./images/e5124aba300efe93c25e400fb8188237.png )

输入一段 range，结尾下标为 -1 代表到最后。lrange list1 0 -1 就是查询 list1 的全部数据。

接下来我们再来看看 set：

![](./images/dde8022b9ff18b3b8e8423b297532e65.png )
...
![](./images/be2deede40cb9e1eb19adc51a613714b.png )

刷新之后可以看到它只保留去重后的数据：

![](./images/879ef6c04fcb4624a246e790c166210c.png )

可以通过 sismember 判断是否是集合中的元素：

    sismember set1 111

![](./images/60ad27d63b345d26309958402d75e917.png )

    sismember set1 444

![](./images/4380cd730c1ba54f6bed0fb6d2fe843b.png )

set 只能去重、判断包含，不能对元素排序。

如果排序、去重的需求，比如排行榜，可以用 sorted set，也就是 zset，：

![](./images/79faf499d00592ab8ee4d58dd6de24cd.png )
...
![](./images/21578b40d13beb1f1cc82696486f1e4e.png )

会按照分数来排序：

![](./images/14f2df0dbdd1564ea248dbbf64973eaf.png )

通过 zrange 命令取数据，比如取排名前三的数据：

    zrange zset1 0 2

![](./images/47a83a7d2a1131032fcb33610137eb79.png )

接下来是 hash：

![](./images/9bee8f1aee845fd340f313087b002512.png )
...
![](./images/326fc6d12051059ea2e6dc43564ef5f3.png )

![](./images/69f2bd21029fbb657f77ca8e92d6f8e5.png )

    hget hash1 key3

![](./images/90a8e99d897cdf0966c259f270f6d369.png )
...
![](./images/c9d905847352b7025315916e962b111c.png )

你会发现 redis 实际使用 zset 存储的，把经纬度转化为了二维平面的坐标：

![](./images/7e519cec047448d1e82ccee070c5c683.png )

你可以用 geodist 计算两个坐标点的距离：

    geodist loc guangguang dogndong

![](./images/9d93dea695c1845ba34fc2c7f12e972a.png )
...
![](./images/55367c0976be86de12cf291616974a5f.png )
...
![](./images/38c8e19b3beb78d915bd8e37761b8673.png )

等到了过期时间就会自动删除：

![](./images/41152729bdc52a301e720b8824791892.png )

想查剩余过期时间使用 ttl：

![](./images/0d2eb857dc3329045c0bf0f955346957.png )
...
![](./images/24ad1f1c44d119e7ade0852a5338db47.png )

可以看到它确实把数据保存到了宿主机，这样就不怕再跑一个容器数据会丢了：

![](./images/50453a7ce5868cbef8b7e0044df7be05.png )

![](./images/3a4fa3d8fd5393014972f0202522f261.png )
