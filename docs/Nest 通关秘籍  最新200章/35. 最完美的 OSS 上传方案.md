...
![](./images/c528490063eaa2e0186db97cd07ee0f7.png )

而 OSS 服务的存储结构是这样的：

![](./images/8d083b2c58376237ba07063e0e86384e.png )

一个桶里放一些文件。

阿里云 OSS 的控制台也提到了对象存储没有目录层级结构：

![](./images/097fa65c9c76734506db68bd5641b4bc.png )

但下面明明是支持目录的呀：

![](./images/3754aa68a75e597e4112d1d3cf75a878.png )

这其实只是模拟实现的。

Object 会存储 id、文件内容、元数据三部分信息：

![](./images/457aee016b2273f648c7840e587de3b4.png )

阿里云 OSS 只是用元信息部分模拟实现了目录。

就像打了个 tag 一样，并不是说文件存储在这个 tag 下，只是你可以用这个 tag 来检索文件。

![](./images/7699f5c2dcc19e7829c6d61b2d93d5ef.png )

除了对象存储 OSS，阿里云也提供了文件存储和块存储的方式：

![](./images/83dee0914263fd9053a4d0a591ea8745.png )
...
![](./images/2ced5c03788ade365596e545eee9d45e.png )

我买了 40G 的 OSS 国内通用资源包，花了 5 块钱。

然后我们创建个 Bucket（桶）：

![](./images/f4cfc806c354ee257035bd6ba090b2f2.png )
...
![](./images/4a6d51a9869e9049927d89b07056ac93.png )

有的同学说，不是静态文件要在全国各地都能访问到么？存在北京的服务器会不会访问速度慢？

这是 CDN 的活：

![](./images/70800c3aa00653bcac1bdc50cebe498d.png )

接入 CDN 后，访问该域名会走到云服务的 DNS，然后返回一台最近的缓存服务器的地址，这台服务器会从源站拿文件来缓存，之后就不再访问源站。

这里的源站就可以是 OSS 服务。

![](./images/47b2947eab89bb54eb5ddbc2b19e42e7.png )
...
![](./images/42d5a06a5ed7717d90d2fb8fc73e9290.png )

点开可以看到文件详情，用这个 URL 就可以访问：

![](./images/efb7e9e5e979408d920a54b6fede7f84.png )

当然，生产环境下我们不会直接用 OSS 的 URL 访问，而是会开启 CDN，用网站域名访问，最终回源到 OSS 服务：

![](./images/b218c4185ca674d3fed1d93cb5d6f66e.png )

![](./images/ce02ba369fc2a4481f7aaeccb7548fd1.png )

在控制台里上传很简单，那如果想在代码里上传呢？

官方文档里有示例代码，我们试试看：

![](./images/c4df63c58b3f01a5383f6583617967be.png )
...
npm init -y
```
![](./images/aa79253ae7a22c5f4ce4d2f877d658e1.png )
...
![](./images/79f7befb7cbc33c9992015c502b246f8.png )

这里的 accessKeyId 和 acessKeySecret 是什么呢？

本来我们身份认证都是通过用户名密码：

![](./images/9ef5de43a285e917edab1a8778dab31c.png )

但这样不够安全，所以我们创建了 accessKey 用来代表身份，用它来做身份认证，就算泄漏了，也不影响别的：

![](./images/ad9cd3815b8f16a65277ad5a440010a2.png )

我们创建个 accesKey：

![](./images/a47d63cc1668ff86b9aa0043a77cee1c.png )

![](./images/06c66550456b150ca88b3fe50cdc3790.png )

创建完成后，拿到 accesKeyId 和 accessKeySecret 后运行代码：

![](./images/dfe8e4beb2ddf307a6eac2b7c5d63398.png )

这里的 mao.png 是这样的：


![](./images/4f022159b1fed4005af8d8a9fb6a036c.png )

在控制台可以看到上传成功了：

![](./images/edea4d9392b947b0ce99af2c382b2922.png )
...
![](./images/be106fedd04ed41eb04c608c4f3ad96c.png )

让我们不要直接用 accessKey，而是创建一个子用户再创建 accessKey。

那我们就创建个子用户：

![](./images/eee99e4148a857afb74b495704a143f5.png )

![](./images/6c2132ebb305d03c34b4db5fbabdf46b.png )

然后用这个 accessKey 的 id 和 secret 就好了：

![](./images/6ff4b795acad3cb28ce1da4d26125a49.png )

但你直接换上它还不行：

![](./images/260e2a70cb47acd4d13e3e8f2b32e351.png )

会提示你 403，没有权限。

需要你授权下：

![](./images/a76e4f9e7bf48048ac6a2934c8bbd030.png )

新增一个授权：

![](./images/9e93a5ca92a3350c16d036ee4bdf2b1d.png )

把 OSS 的管理和读取权限给这个子用户：

![](./images/ca7a61dee34fd09ea3e080b1ac74461c.png )

然后再试下：

![](./images/f655175fe8d4d677a2cfe009558f8d08.png )
...
![](./images/4646fff11e1e2c7a00c55416e72fb7c1.png )

那万一泄漏了不就完蛋了么？

但是如果创建个 accessKey 用它来做身份认证：

![](./images/f84744c0bef62504304f2b77d7dcd95a.png )

就算泄漏了我也可以禁用啊：

![](./images/e193b1b5e6cee3aaa926a4ce4cdf616e.png )
...
![](./images/e1f21812a57ef8731a4d25d39d586988.png )
...
![](./images/27cb39b89b01cdbd1184062d08e404cb.png )

具体怎么用直接看文档就好了，这里我们就不试了：

![](./images/6a9dc1adfa1bb2c5aac460cdcbbe1a00.png )
...
![](./images/ce54ef4f30a04c74437866b7237eb5ad.png )
...
![](./images/eb9840eb6943b7ebf04751d87aea91b6.png )
...
![](./images/8227ef0fbd301b941206248428f728de.png )

这些代码不用记，文档里都有：

![](./images/dd81b8ef3a1f94ce11c06d900a3f5845.png )
...
![](./images/3dc05125a8122e5136a6c429042da5f4.png )

这时候你上传文件的时候会提示跨域错误：

![](./images/1819cbf5c1b57c974858c6176c23ec26.png )

我们在控制台开启下跨域：

![](./images/15105835f5690d794dfa67c5872ff34b.png )
...
![](./images/dec1b95754313cc173ec2b2ea5697df8.png )
