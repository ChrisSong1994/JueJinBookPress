...
```
![](./images/977b07d0691f86bac4e9ba010239bf3d.png )
...
![](./images/889b8f284ae160b6d649109721124532.png )
...
![](./images/b9a97d63fef599e465159639c70a62f7.png )

在 mysql workbench 里也可以看到这个表：

![](./images/35d1d8e3840302162d293be998b3c558.png )

把插入数据的代码注释掉：

![](./images/c4d07880890a0a80390a9315c96dea06.png )

改下 Entity：

![](./images/844ff2e6d60c8f910b5689674956003b.png )
...
![](./images/afc7e09863002aa483b4f913d7703f6c.png )

在 mysql workbench 里可以看到，之前的 age 列就没了：

![](./images/faa69e83f1d0fe247d6ab2e8bf8de45e.png )
...
![](./images/edb30bfdf0fb9e6016e57dff2534903c.png )

生成了 “时间戳-Aaa.ts” 文件，这个就是放迁移代码的：

![](./images/e34efc1f3d88dfbf8e30e5ebec09726e.png )

迁移就是 create table、alter table 这些。

我们在 mysql workbench 里导出下建表 sql 语句：

![](./images/b5c4548e7330219e4cdd581b05051ced.png )

点击左侧的 Data Export，选中要导出的表，指定一个 sql 文件保存位置，点击 Export。

可以看到，生成的 sql 里就包括了 create table 建表语句和插入数据的 insert into 语句：

![](./images/98c85498313903764680a5a9c27478e2.png )

我们把建表 sql 拿过来：

![](./images/1fddf1e36f70544b124c4975d8a222c8.png )
...
![](./images/39508684e5fe9682a1a0ee5856d9046d.png )
...
![](./images/828d21cdf2c36ba43497f1cc1f1a314a.png )

把 index.ts 注释放开，但 age 去掉：

![](./images/3d5c03cbe280e88d5d586f9448b86310.png )

然后再跑 npm run start

这时候因为 synchronize 关掉了，不会自动建表，所以 报错了：

![](./images/a727a4723464f8e81100268f391e0473.png )
...
![](./images/c5d2ccff1d7f1ec93e2120ecd28568fe.png )

可以看到，执行了 migration 里的 create table 语句。

这时候数据库中就有这个表了：

![](./images/85d2e1cc36f3985125859cffe2d5e3ae.png )

并且还在 migrations 表里记录了什么时间执行了什么迁移：

![](./images/aa79aee768bf5d39f2d37fdbf29e2aa7.png )

这时候再跑 npm run start

![](./images/d0afddd791a0936e6c7b7f329ed9f7ae.png )

这时候 insert、select 就都成功了。

![](./images/167a4d0d186eed0053825801d2dbeb95.png )
...
![](./images/d3d671e2e9983835284beff51ddc2370.png )

把之前那个 migration 也删掉：

![](./images/a72bc7570abfa13611c67920d12d52ee.png )
...
```
![](./images/f9a9e1124de7be9a8918a63a75a06615.png )

生成的 migration 文件如下：

![](./images/f914cd6f78a68830fd13421c6e899fd1.png )
...
npx ts-node ./node_modules/typeorm/cli migration:run -d ./src/data-source.ts
```
![](./images/7c298f6cbbaf130851bcf09f4dfac29e.png )

然后再跑下 npm run start：

![](./images/54182b78a08ecd3c1bc7095cbb9b142e.png )

![](./images/af5aa1e9dee3064a4d46f8fea044754b.png )
...
![](./images/91d0a0101452cfff6ee395175159d190.png )
...
```
![](./images/91646bc79f8da6c044b25fa4f51e554f.png )

![](./images/441b36ad6cce27d4f33e7a6727a5de1c.png )
...
```
![](./images/b3f51d7012b04a843e975598b04b7dfe.png )

在 migrations 表里记录了 Aaa 跑过，所以这次只会跑 Bbb 的 migration。

![](./images/d607ce4e56861fc52f94563bc43abf61.png )

![](./images/b398537cf6092733161296f71bf59429.png )
...
```
![](./images/a2114020bae7ada106a5f825e402714f.png )

执行 migration:revert 会执行上次的 migration 的 down 方法，并且从 migrations 表里删掉执行记录。

![](./images/ef237e9d4312ab171e1f0d93455e6e2b.png )

![](./images/a6b1477468ff4e05d892b64f2a5ed681.png )

![](./images/701e7a72bd3c2d301d0a7ac4cc5eeea9.png )

再次 revert，会撤销上一次的 migration，删掉 user 表：

![](./images/27a8da61d65039593a57952218b8e027.png )

![](./images/083d39919ff6204e1699142c6d3f7c05.png )

所以说，每一次的 migration 都是可控的，可以手动执行、也可以撤销。生产环境我们就是用 migration 来修改表结构，而不是 synchronize

此外，每次都输这么长串命令也太麻烦了，我们可以把它封装成 npm scripts：

![](./images/ddbb3a7620f3b2b0fec5a968b83b3ae2.png )
...
![](./images/d086c03192c36954ffdb3dcc8fe9e966.png )
