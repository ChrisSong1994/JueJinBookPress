...
![image.png](./images/8a61526dbf91b4618f08a164165a89f3.webp )

但有的接口，不只需要登录，可能还需要一定的权限，这时就需要鉴权（Authorization）。

比如管理员登录后，可以调用用户管理的接口，但普通用户登录后就不可以。

![image.png](./images/fdc230dd73faf06fd6dc50e23cb87f36.webp )
...
![](./images/5fe41082d05610d12f5934a70b89ad39.webp )
...
```
![](./images/0d2a516845e8bdf901f24035df12e5c6.webp )

刷新可以看到这个 database：

![](./images/61b2243625eaf8a23a8df456c9a0656f.webp )
...
![](./images/cba33ca743331c88537ae14c770d903d.webp )
...
![image.png](./images/592132ad843ec3d7a3203bb89c953bee.webp )
...
![](./images/71ff586caff1b99e2f46c66efb48da81.webp )
...
npm run start:dev
```
![](./images/5133e5ac7db3cda1b8cc7bf3cd288023.webp )

可以看到生成了 user、permission、user_permission_relation 这 3 个表。

并且中间表 user_permission_relation 还有 userId、permissionId 两个外键。

![image.png](./images/4f6ef5ca1dfc9e5eba72882a4a760adc.webp )

![image.png](./images/4fc97c6c700ce267642f484413bdd25c.webp )

![image.png](./images/92b0673580702ba715bc64c42f8f4d0e.webp )

![image.png](./images/acae3cf656435ed69e06772d4c7e78c2.webp )
...
![](./images/c479f41335b73d8c170ef8ad54d0b2d4.webp )

服务端打印了一堆 sql，包了一层事务：

![](./images/1c79cdbd9aedc1307bff683fba36264e.webp )
...
![](./images/31a94b5fdec701e51a6841929cbc3f03.webp )

user 表插入了 2 条用户记录：

![](./images/e875f7e7ffc8bcf96b692bb056e8fa2d.webp )

中间表插入了 8 条记录，两个用户各拥有 4 个权限：

![](./images/44a897b0cee0ae274ee474e9e746d79f.webp )
...
![](./images/4c1f7ed18d313341766d616e03d8149f.webp )

然后在 postman 里测试下：

ValidationPipe 不通过的时候，会返回错误信息：

![](./images/84d404f7646790cf53f5e5ee32cd8874.webp )

ValidationPipe 通过之后，就会执行 handler 里的方法：

![](./images/aa71e9e94d5121870b99f5dfc736d6d0.webp )

![](./images/a6c1a4055a0f4e040d5a631fd4e24a4c.webp )
...
用户不存在：
![image.png](./images/fc727c525b0997e9751324f91bea892a.webp )

密码错误：

![image.png](./images/031f6de7b60a66d25359fa3e7af2e441.webp )

登录成功：
![image.png](./images/9ff0d3a62c26c0f0a33311aee4f5e307.webp )

![image.png](./images/9885f5fb2a70e5b49d36ac9b9b1fe838.webp )
...
nest g resource bbb 
```
![image.png](./images/d7cfbfecee05de9080a5002f0d95e012.webp )


![image.png](./images/63bdeb6d935e319d93b0ec1c48831ca3.webp )

现在这些接口可以直接访问：

![image.png](./images/b56f2819274930479332a71bb32cac20.webp )

![image.png](./images/e9f301302a6abc4210c37ad7d02bd3c3.webp )
...
![image.png](./images/2d691c88954dbc7c4381f4a89d2e2919.webp )

利用同名 interface 会自动合并的特点来扩展 Session。

然后给接口都加上这个 Guard：

![image.png](./images/483bd211eccb5f86fde31d74740c3195.webp )

![image.png](./images/e5ad451c8abd7a3eef7bd230601b5fc9.webp )

再访问下：

![image.png](./images/8fb9a7d1c88e3ed1337d67b8588da05a.webp )

![image.png](./images/ebfaa64a1e6fa5cee1766c7ff7b8fd45.webp )

在 postman 里带上 cookie 访问：

![image.png](./images/96a6cb3ddba23b22901d107903c3dee8.webp )

你访问登录接口之后，服务端返回 set-cookie 的 header，postman 会自动带上 cookie，不需要手动带：

![image.png](./images/a659ec471db07ff8ae33322c86e7ae0e.webp )

行为和浏览器里一致。

这时候再访问 aaa、bbb 的接口，就可以访问了：

![image.png](./images/b6e03a12946aa9a5263795cefd41cd29.webp )

![image.png](./images/d6cb3206d98a637076caa4f90bdf3449.webp )
...
![image.png](./images/01be99c140ba9ec442d382f89da36363.webp )
...
![](./images/83c009da84c738c70b21cf2b1510d239.webp )

然后在 /aaa 的 handler 里添加 PermissionGuard：

![image.png](./images/7195bf7498dc603d69d80124bee2f4c5.webp )

postman 访问下：

首先重新登录，post 方式请求 /user/login：

![image.png](./images/bd1c3dc99d0d4bb35d920eecc64700fd.webp )

然后 get 访问 /aaa，postman 会自动带上 cookie。

![image.png](./images/5c51cc170ef78e1fa5d9a4046c6a3a3e.webp )

服务端打印了 UserService：

![image.png](./images/44e97eb18cf1b17e4c6070c5a06221ad.webp )
...
![](./images/b0ea34384936a473ba851b5693a5a8fb.webp )

然后请求 /aaa 接口：

![](./images/bbdb7faf673aca2b72e19be51afefcec.webp )

服务端打印了当前用户的权限信息：

![](./images/912bf6d0710c80f8c7bbf144b9cd8f33.webp )
...
![](./images/15bbfeb63fa7c9305d8d17339224c6ce.webp )

给 /aaa 接口声明需要 query_aaa 的 permission。

然后在 PermissionGuard 里通过 reflector 取出来：

![](./images/74752f44a4d782bf4c6caf5003f09358.webp )
...
![](./images/8be6a5f6d6f49e842956dc10fb8fb33f.webp )
访问 /aaa，会提示没有权限：

![image.png](./images/5863487cf5589895c023e4be3cc8e7dd.webp )

然后登录东东的账号：

![](./images/a166f4a4d81e269cd6175c4712636e19.webp )

然后访问 /aaa：

![](./images/0678941456a8ab11461b4dc101645e8f.webp )
...
![image.png](./images/a00afd77f195707a090785d667210e9c.webp )
...
nest g module redis
```
![image.png](./images/5615ea22565323c3132914263e257590.webp )
...
![image.png](./images/e94232518d7270e406e6c8896f25e3ab.webp )
...
![image.png](./images/21b71f07cef38fc3b8522b1060e8dadf.webp )
...
![image.png](./images/b36f36ba39a1d21d257a231ac8a35aa3.webp )

然后先登录：

![image.png](./images/67fd1ce6bd08b96b2bd37c65dad5af89.webp )

服务端打印了查询用户数据的 sql：

![image.png](./images/149b29d22b1578094089ec1371b7ea66.webp )

然后再访问 /aaa

![image.png](./images/9b4be767cf04aa4f49f41327fe8d8956.webp )

又打印了 2 条关联查询的 sql：

![image.png](./images/dc282f342e887b8eb4d0ccf0116ec1d2.webp )

我们去 RedisInsight 里看下：

![image.png](./images/75ec2e00a9086ce00c784bbd0d104de1.webp )

可以看到这条缓存。

这时候你刷新多少次，都不会再产生 sql 了：

![2023-06-22 17.16.43.gif](./images/20b0ab14f6e239ab4a3efcada3dbf0b3.webp )
