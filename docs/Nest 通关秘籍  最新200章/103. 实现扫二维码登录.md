...
![](./images/2a155868251bea3086343c9fe81c03ab.png )
...
![](./images/35c9ddb5e3ab320aec4ecc6e1073cca5.png )

![](./images/1718ca6f3aa56004b5ee22be6c605306.png )
...
![](./images/072ba98f1b3e46b3c32d6688b3153a19.png )

![](./images/ad27b1f0dbfa6a125e7162905b84b958.png )

可以看到，二维码的内容是一个 url，如果在手机浏览器打开，是这样的：

![](./images/a363224635a751fd3f72ba6e093dcaa7.png )

会让你下载 APP。

而在 APP 里打开，就是登录确认界面了：

![](./images/97d576558d4342fd584a7be2a7dfc03b.png )
...
![](./images/b2f071c9207d92a84d82985355a4d948.png )

扫码后会进入等待用户确认状态：

![](./images/4bd3028105d03f97c9fab01cb2114f0a.png )

确认后会进入同意授权状态：

![](./images/a7fab46f473f09ae718d6804f99b90d9.png )
...
![](./images/308dc6e237dd6bd61b7561364daccb0f.png )
...
![](./images/c9f2447ebb9e0197838879babfaf4462.png )

扫码后会变成 scanned：

![](./images/bbc5837785c739c53c365caf3f97411d.png )

知乎也是一样：

![](./images/a221e6b7405b92465f6582282112a3cf.png )

![](./images/f58cdb8d3453bb0399c076a0ab734b28.png )

这时候，手机会进入登录确认页面：

bilibili 的登录确认页面：

![](./images/8788685a2758d3c41b0f82d731f6d256.png )

知乎的登录确认页面：

![](./images/abb51b6d2e8f91db6853fb5533681002.png )
...
![](./images/08eb7080167d7b9e20eb9c17e81a7429.png )
...
![](./images/494f87ea52f83f8fd9d748c517acc16d.png )
...
![](./images/7ad9154e351d2cd79f45865630a32f1b.png )
...
![](./images/62a38c5fd1c5020c74e673a2d04c6536.png )

浏览器访问 http://localhost:3000 就可以可以看到 hello world，就代表服务跑起来了：

![](./images/6f591a891c6b7aa11c74b6cccf6ffd50.png )
...
![](./images/efa3699a1dcac2ab0a1e254ffe22bf5d.png )
...
![](./images/8358eac5f6a177b65345eb256ee194c6.png )
...
![](./images/7f5fcb12e6727f4b4acbf5bc6674700f.png )
...
![](./images/66644824a212b8eac4057f1d48a7b5c1.png )

我们用在线解码工具解码下看看：

![](./images/772c14644e95721d7d384550b4ecee98.png )
...
![](./images/3e5bf7c95a103bce0753b82fbc9d46ed.png )

如果用手机浏览器扫这个码的话，打开的就是下载 APP 的页面：

![](./images/5baad5ed7d84a1cdbe63ed2915b69623.png )

而如果用掘金 APP 扫码，扫出的就是登录确认页面了：

![](./images/c6c6d01fe0b31b5a2d352fe52f499f23.png )
...
![](./images/50aa49d0f1f9dbd0d023f5497604b2cc.png )
...
![](./images/5e12e9f4e7a80fbf30ee96c1a9df2a9f.png )

让电脑和手机连接同一个 wifi，然后在手机的 wifi 设置那里设置代理：

代理的 ip 是电脑 ip，端口号就是 charles 代理服务的默认端口 8888

![](./images/9b5b05a6414a04d5c6d10fd144ba4ad4.png )

这时候电脑上会收到连接提醒，同意下就好了：

![](./images/60d029f8a3051b7f270ce745aec1d8ea.png )

然后手机就可以访问电脑上的 nest 服务：

![](./images/ac8a4b6ce0aff9368e50e3b754dba49e.png )

看到这个 hello world 了没？

这就是电脑上的这个 nest 服务返回的：

![](./images/f058771eb4fef0c24f791ccd374b92b9.png )

那个登录确认页面在电脑访问是这样的：

![](./images/ac5967f2eca175d5a108a98c38b5f1d6.png )

然后我把二维码的内容改为这个：

![](./images/e4f9b8771068889f80af3a9a274ad632.png )
...
![](./images/ca92523545f7a53b2f496c566583a361.png )
...
![](./images/85fd1f2bdab97d87fdcbea26d366495f.png )

然后我们来实现剩下的接口：

![](./images/ec15c20ddf69c38389c8338a11ee2668.png )

生成二维码之后，要在 redis 里保存一份，这里我们简化一下，直接用个 map 保存吧。

![](./images/3305d45796458aa3c695d99401678515.png )
...
![](./images/ce520529673818dc393388eadf66a78b.png )

然后访问 /qrcode/check 拿到这个 id 的状态：

![](./images/0af4f06e07f9d7a53edf94c6316fe0b6.png )
...
![](./images/a123de6f0a73e21822c8b911c2c3f453.png )

然后调用 qrcode/scan 修改状态，之后调用 qrcode/check 查询下状态：

![](./images/928c5dbcf843c6283a724fc246947ea8.png )

![](./images/123e697a9cd80102e809c8869e25a3d4.png )

同样的方式测试 qrcode/cancel 和 qrcode/confirm 接口：

![](./images/e5336f5e77f2d4234d6c38eb1f024d92.png )

![](./images/e7a896d6938e888b07d8ff701b49b913.png )

![](./images/0f5ef0ff0fcabd67a8a475ff91530477.png )

![](./images/a2f45f816974ce4ed755db70d0be93c8.png )

如果 id 不存在，会返回 400 的状态码：

![](./images/f32f7885f852632c4ef95089eb2303d6.png )

![](./images/4ae6e049d76d522e6726c6e7e1d06e6a.png )
...
![](./images/d7a9f0cf5f7658b7d148d5aa2ddbc2ed.png )
...
![](./images/bc26cadb95215ab7abd6ed3d22a52371.png )
...
![](./images/314fc35cc53ef04a7a1e0e75b4421f15.png )

然后确认登录，这时候 pc 页面变为了已确认状态：

![](./images/c59b6c92ea6e166c7184fcf0d559ccbc.png )

![](./images/68e022183f63dde578dfc7eb0715ed93.png )

如果点击取消，那就会变为已取消状态：

![](./images/9e9270bb58d63b9e8464eb98203b3a0d.png )
...
![](./images/d258dc633720d18e899870f9bc17f845.png )

然后实现登录接口：

![](./images/8a889ec0f9ff8231936937086c80228a.png )
...
![](./images/1ac5e875c12ffe2c4354f63d4dc41c99.png )

![](./images/c7a4e1a3e52a70642ce5df1461fa81f7.png )

![](./images/e2d76c9770fbdb40d9a472e3ee7dcec8.png )
...
![](./images/1d9f92edd3cadf7ed4f256497576c243.png )
...
![](./images/35daf731ff11b43955d966760fd1fe9e.png )

这俩按钮分别是登录不同的账号，拿到 token：

![](./images/a096b403b5a12371cc3a9148d04facfd.png )

访问 confirm 接口时带上这个 token：

![](./images/25e6ffdf0b09f1c9f964efcc34a7f4fa.png )
...
![](./images/aaf602d6dbd62ccf237a59ccc92e62aa.png )
...
![](./images/a958df1e70e84947ef3e24836f561193.png )

加上登录之后，我们再测试下：

![](./images/0df2343118fc64124a559826b9755e1e.png )

手机扫码，点击登录光的账号，然后确认登录：

![](./images/35102538cb15fd52b5f294e2a2603ea1.gif )

![](./images/8d4b1b9e051f511ac990c2c83eba27b8.png )

刷新页面，重新扫码，然后登录东东账号。

这时候 pc 网站就显示了当前登录用户是 dong

![](./images/e1e36714c9a30dd8903e7d8b9cd258d9.png )
...
![](./images/b6afd27602fa8fb09cc7468c8010f9e9.png )

对比下掘金的扫码登录流程：

![](./images/65de663e130c3487115ceaab286886cf.png )

![](./images/8db4b54310905f1391a7efc2d8d7a031.png )

![](./images/2e78207e03562daa6a937f7688d00029.png )
...
![](./images/55f85618764a2e2d48e7fe32390c3d2e.png )
