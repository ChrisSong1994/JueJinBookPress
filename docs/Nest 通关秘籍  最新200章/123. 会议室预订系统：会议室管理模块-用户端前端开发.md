...
![](./images/6b4f4e258c9b917bb943598427025a85.png )

需要在 / 下添加一个二级路由：

![](./images/0fc227f1b142c244ed5df0fc985150bb.png )
...
![](./images/47c53f872a54bf36d140f95f334b25cd.png )
...
![](./images/2298df17dcdf7b0dabc7f3ee2f023b70.png )
...
![](./images/4eebfc4c16be942857969f671a70c796.png )

其实会议室的接口都是需要登录才能访问的，当时为了测试方便没有加，现在加一下：

![](./images/b2da39f00a176e3e33dc54196ae7e51e.png )
...
![](./images/1eaff7a35c2a5b1ce2575cb0258e8441.png )

这时候直接调用 delete 接口就会提示需要先登录了。

然后我们登录下，拿到 token。

![](./images/9b215fbe541bf937ce719862f4e78402.png )

把它复制到 swagger 文档这里： 

![](./images/8d126d364931bafaca47ad8bec99e2a6.png )

然后点击这个 try it out：

![](./images/2d07649dbbd0d4b3c4e8d77ff6e6ba4c.png )

数据库中现在有 3 条记录：

![](./images/7c358f7c210814f04b754a1d803dd0aa.png )

把 id 为 10 那条删掉。

点击 execute：
![](./images/10b9ece8eb9c2f702545eecc195c0719.png )

swagger 会发送请求，下面会打印响应：

![](./images/b098e70514916364eed5547b8e2e6261.png )

这时数据库里就没有这条记录了：
![](./images/0e1201adb6f0972a1251ac039fc6ff8a.png )

可以直接在 swagger 文档里测试接口，不用 postman 也行。

然后继续写下个接口的 swagger 文档：

![](./images/49bd35debbeb6a34f829a6db7cf64938.png )

这个接口的参数也是用 @ApiParam 标识，但它的响应不是 string，而是 MeetingRoom。

而我们现在并没有 vo，没地方标识属性：

![](./images/b99642e11facb36a7af6bc6e88e90d20.png )
...
![](./images/0b46232bcf1117ee06d7a8585d08a2d9.png )
```javascript
@ApiBearerAuth()
@ApiParam({
    name: 'id',
...
![](./images/3368ba36f001b4da0d83807052156636.png )

接下来是 update 接口：

他有两种响应：

![](./images/b2010bfde60180410ed57be7b1ec5aa3.png )

分别写一下：
![](./images/bff26f105fc7b5128e82a02530a9b9c0.png )
...
![](./images/43aa272de348c07f5b3009ba09c7c61c.png )

因为 update 的 dto 继承了 create 的 dto，所以那里也要加一下：

![](./images/c91235f2138d6242ac80b63e6a94b139.png )

这样 swagger 文档就对了：

![](./images/2050db69ceace124b1548f88713e621b.png )

然后是 create 接口：

postman 里调用下是这样的：

![](./images/c37a40ca1b72d2fbb5c903c5db3fd94d.png )

![](./images/44363e69ab81da434d41930e3f123f1f.png )

所以 swagger 装饰器这样写：

![](./images/621854d1e4d0a06b392a1092c75f410d.png )
```javascript
@ApiBearerAuth()
@ApiBody({
...
![](./images/dc184eee5c4a0aa65f4fec2685faf636.png )

然后还有最后一个 list 接口：

![](./images/9fc9659990f21a8d4f0dad016ab34171.png )

它的响应是这样的：

![](./images/97372eea509d3d811ba33280db95c30f.png )
...
![](./images/fde61ec403c0ee266dbef693d110c988.png )
...
![](./images/b0c7158cdc2aa8ed4bf0518bc21932f0.png )

![](./images/17264779e38af5eb3684a6b21789d6b4.png )

最后，在 controller 上加上个 @ApiTags，把下面的接口分到单独一组：

![](./images/c293090117972cb3872e1e2786d9cb89.png )

![](./images/a5009dbd726e2584c39d3af3ba809f56.png )
