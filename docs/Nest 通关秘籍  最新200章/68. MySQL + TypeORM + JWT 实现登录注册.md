...
![](./images/08f3c12d5206c4f41a2019294dc0b19e.png )

刷新后就可以看到这个数据库了：

![](./images/17852f13c49e1cf11767c2d354d165f9.png )

然后我们创建个 nest 项目：

    nest new login-and-register -p npm

![](./images/25be0b73106abb52a8a275433626a227.png )
...
![](./images/c42e2d82a884cd5123c05aff538b9161.png )

引入 User 的 entity：

![](./images/c74d65f9b0e1240c2bde3c296fc6ab97.png )
...
![](./images/db88105cb982dc5ed1556a85d21f36ff.png )

可以看到打印了 create table 的建表 sql：

![](./images/fe8c9b1f7a1bc895beae7f8d721acda9.png )

用 mysql workbench 可以看到生成的表是对的：

![](./images/d72f2716142085a7805ebdeee9c9e8bd.png )

然后我们在 UserModule 引入 TypeOrm.forFeature 动态模块，传入 User 的 entity。

![](./images/4cbac1da6b27df1e310cd275bcf0a92f.png )

这样模块内就可以注入 User 对应的 Repository 了：

![](./images/df0aea7cf81a1907795e9e88c3c544b6.png )
...
![](./images/b4c403b87c185c9e52c2669d0d209b01.png )
...
![](./images/6c5018dcd6a5c2f55957ae9008465955.png )

我们先在 postman 里测试下：

![](./images/eddfc292773d85396bae66de860580cb.png )

post 请求 /user/login 接口，body 传入用户信息。

服务端打印了收到的 user：

![](./images/5783ef78be8444c664dfae83b0737e59.png )

然后 post 请求 /user/register：

![](./images/f20f8c2d53c4a991eb1f60855b836f6f.png )
...
![](./images/2806b1c8dd94678adb8b58c503626ad3.png )

我们测试下：

![](./images/32c610c3ad21ef62de45d49bb0837c0b.png )

服务返回了注册成功，并且打印了 insert 的 sql：

![](./images/7c583aaf3913c4c2efc89458fd3f41b4.png )

可以看到，数据库 user 表插入了这个用户的信息，并且指定了 createTime 和 udpateTime。

![](./images/04cb0ff8b041019ed5e45aad6ddb0209.png )

然后我们再次调用：

![](./images/b9008129bdd087de3c3b81b68b72c6d8.png )
...
![](./images/fee749b4d9c47ffff300dd0f206a5562.png )

用户名不存在：
![](./images/7ae3513dc1eef8106d3b97a409931dce.png )

用户名存在但密码错误：

![](./images/3b8254e5e9456709470f65b769e7ddea.png )

可以看到，服务端打印了 3 条 select 的 sql：

![](./images/ea64bee3a6ffef8ac0cc24c24c9fa137.png )
...
![](./images/dd6125fdee0e3d3188171c2add1fa268.png )

global:true 声明为全局模块，这样就不用每个模块都引入它了，指定加密密钥，token 过期时间。

在 UserController 里注入 JwtService：

![](./images/27b704e27af08c827bfde17b8e571f47.png )
...
```
![](./images/36f365f8b8d9b00da9091d32e4d232a3.png )

再次访问：

![](./images/ccfa251842caf63c97c86687bf13dcd4.png )

![](./images/c9fa1ce260c373dfb88295a55a4d774e.png )
...
![](./images/ee31d4f9d71450ad63d1d9a50a4a2f45.png )

![](./images/d8a1e69cb085d2df9400f4d4ccd1b2c2.png )
...
![](./images/a4424f01ce58106459c78cf1a7fa661a.png )

我们先登录一下，拿到 token：

![](./images/25ff432871c524365bd55345e63b6910.png )

然后请求 /aaa 的时候通过 authorization 的 header 带上 token：

![](./images/cbfec11caa8c7f5cd56df1e857be844e.png )

访问成功。

如果不带 token，就失败了：

![](./images/8fd84d7dd1b7eec0934dc13944187381.png )


![](./images/62be3a4ca16bb1b9e57290dab1f04cbb.png )
...
![](./images/2f0a0ca99883823ae3c3747a0d29ef5c.png )
...
![](./images/344ba62ac68f9d43207263da707b6215.png )

![](./images/f12b863241488968322ae06a00374b2b.png )

![](./images/0eb973a118149afc7ab6828dbc1f9f54.png )

ValidationPipe 生效了。

这样，我们就实现了登录、注册和鉴权的完整功能。

![](./images/493c65c86c64e1eed8687984b7b69f54.png )
