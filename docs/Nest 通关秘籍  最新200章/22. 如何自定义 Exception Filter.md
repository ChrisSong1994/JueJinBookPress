...
![](./images/57ba1e7d551b8a861730109a53e36af8.png )

服务端报错时返回 500：

![](./images/53741c72ba251a8fadf3baef9b2338c3.png )

参数的错误返回 400：

![](./images/b18a6a1bdfba17a4f0da15665f230eb6.png )
...
```
![](./images/8e5f9ca3e3965dee7a3006244243f05e.png )
...
![](./images/c1bd1431ab4006462c7e40bd4feb56ec.png )

浏览器访问 http://localhost:3000 可以看到 hello world，代表服务跑起来了：

![](./images/e48232afacc65e853a42cffd44cc46cb.png )

然后在 controller 里抛个异常： 

![](./images/a72aabb86b9d105e984bc57e8ba0b0eb.png )
...
![](./images/7125b79abb821c0e59b16246770a0966.png )

这个响应的格式是内置的 Exception Filter 生成的。

当然，你也可以直接抛具体的异常：

![](./images/19166597702f79dbd43bead284f4f4b9.png )

![](./images/3f787b21e998f027a3e58da6d8cfef39.png )
...
![](./images/84d0869ce980bc207e5f10dfafeb0a5f.png )

![](./images/00c6ae7fb4177c4c6cc399405cecbde6.png )
...
![](./images/1da2885ae37e6151250a6f85208d479e.png )
...
![](./images/953f7a9abf7a72af51d0ac988b9bf89c.png )

![](./images/3586acf8a8a55e63519b2f2d898dd9e3.png )

然后新建个调试配置文件：

![](./images/0372bdbeb10e76e5ffca0c580a8d2b9f.png )

输入调试配置：

![](./images/ac1cebbaea855e6116df2e154c9c0c5c.png )
...
![](./images/68f576cf15c39180b7203da88786156a.png )

刷新页面，代码会在断点处断住：

![](./images/8eb45ce0e1826bbeede10d04502ebe48.png )

我们只要根据异常信息返回对应的响应就可以了：

![](./images/9f9db8b61f52e51325de8dc019f806b6.png )
...
![](./images/b76a9f534231ce3e381799a64645e894.png )

但我们只是 @Catch 了 BadRequestException

如果抛的是其他异常，依然是原来的格式：

![](./images/d566ab4241b179d6691994c7adcde389.png )

比如我抛一个 BadGatewayException。

![](./images/717cb838c3e39109f773ae34f66b0af1.png )
...
![](./images/bcfe920c9503684857a227f15afdb072.png )

试一下：

![](./images/81962f2ca0e56d19375298b3a09aabb2.png )

![](./images/36e678a71e5591bb4297e65a06442e14.png )
...
![](./images/0646be959887c95d3a5f9c8e5d667aca.png )
...
![](./images/97e1c6db8352a8e88249457a20561d04.png )

可以看到，提示的错误也不对了。

因为我们自定义的 exception filter 会拦截所有 HttpException，但是没有对这种情况做支持。

![](./images/01c4eac93216c7b6d3d97387107bc56e.png )

先不加这个 filter。

这时候响应是这样的：

![](./images/0f53026a6f252c878de3b545d97e8772.png )

我们对这种情况做下支持：

![](./images/698454548c85487a1cfe0a9e8d15e4b1.png )

启用自定义的 filter，然后打个断点：

![](./images/db1d1f3fa578f457da55735b2909746f.png )

再次访问会在断点处断住：

![](./images/6a02dbae09bfaf1bfd5e860d458e5594.png )

可以看到 ValidationPipe 的 response 格式是这样的。

所以我们可以这样改：

![](./images/3094b6645d0ba865ad2b10ae53b0972f.png )
...
![](./images/77c1c11bb8b6b95300e3aef65faa04fd.png )

![](./images/c4c0fc4d7347f51014d9f5d9fe685284.png )
...
![](./images/7e6239470fc1a1d67b083556370404ca.png )

不用 useGlobalFilters 注册了，而是在 AppModule 里注册一个 token 为 APP_FILTER 的 provider：

![](./images/ff8f1e3ea0881f60953cb01f5227f497.png )
...
![](./images/ea493babcb4d3f29d7cae5deccec3175.png )

这样注册的好处就是可以注入其他 provider 了：

比如我注入了 AppService，然后调用它的 getHello 方法：

![](./images/de2e05f43867f6496d3d47add3e887d0.png )
...
![](./images/ad7d490d312a9490ef67eba969874133.png )
...
![](./images/7b96f4a5e2c8d757498c0aebd250f8e2.png )
...
![](./images/670671682c78c394abb351f53e1fc917.png )

浏览器里访问下：

![](./images/f324cd68cc58438c43cae682aa3e79ac.png )
