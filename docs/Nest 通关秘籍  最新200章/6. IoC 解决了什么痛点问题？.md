...
![](./images/e4dcc69fb42c9fef9af5354f96e2bac9.png )
...
![](./images/aceb9752ec85f51b8b9675fc632f473f.png )
...
![](./images/53c62671e7cc71bbe4cb5ce9b7ebb85b.png )

选择一个 npm 包管理工具，然后 nest cli 会自动创建项目结构并安装依赖：

![](./images/c72dc774431dfa1042b74b54e1456563.png )

然后进入这个目录，执行 npm run start，把服务跑起来：

![](./images/7aecb94eb016db8df8e0a3eda74b2e0a.png )

浏览器访问 http://localhost:3000 就可以看到 nest 服务返回的 hello world：

![](./images/67b2b3468e27d86fbf05af95ec2d1621.png )

我们看看代码里它是怎么创建对象的：

![](./images/b06d9c5d83fefc3fd3f8eb82a26c98f8.png )

它有一个 AppService 声明了 @Injectable，代表这个 class 可注入，那么 nest 就会把它的对象放到 IOC 容器里。

![](./images/ff54931d73d393558c67a759c6278a4e.png )
...
![](./images/a2a8e0d1e7da0452fc51cd7d1e62465d.png )
...
![](./images/53eccf8ef801f1f7de45f5ca8a164146.png )
...
![](./images/3f2c8190a1d782cf3f4a209663b8a3cb.png )

那么 nest 就会从 AppModule 开始解析 class 上通过装饰器声明的依赖信息，自动创建和组装对象。

![](./images/3900a91c61dbcebb31caca950c21da71.png )

所以 AppController 只是声明了对 AppService 的依赖，就可以调用它的方法了：

![](./images/2bbba41742e5b30e48cf5180554de16a.png )
...
![](./images/daed49df04ac7d9c4f582557b150939f.png )

会生成如下代码：

![](./images/f891af2195fc55f6c6ed82a4cdf3823a.png )

用 nest cli 的 generate 命令生成一个模块。

会在 AppModule 里自动 imports 这个模块：

![](./images/0a4ff43c287a789ba4a1b3f1a69c7ffe.png )
...
```
![](./images/3df5eaeaabc62a1b5c304319d0bc5356.png )

会生成 Service 的代码：

![](./images/1c15d9d6d19c012367818dbfd2eb77f6.png )

并自动添加到 OtherModule 的 providers 中：

![](./images/b18fa1c686691847b6550734de6e6a22.png )

我们改下 OtherService，添加一个方法：

![](./images/97b7dfb5c253372255f0b1a87fd91617.png )

然后在 OtherModule 里 exports：

![](./images/4ca04461799824b98f9c7e67981df8e8.png )

那当 AppModule 引用了 OtherModule 之后，就可以注入它 exports 的 OtherService 了。

![](./images/d5c07f020c796e17b6fba6b42f2957ed.png )
...
npm run start:dev
```
![](./images/cc0f06a9f29b0dcdbf6dc58f623d1751.png )


浏览器访问下，可以看到 AppModule 的 AppService 调用 OtherModule 的 OtherService 成功了：

![](./images/2969741e6c3b11ba15df287925092bcd.png )
