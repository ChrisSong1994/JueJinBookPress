...
![](./images/5c701df5ac90302660a3b943068d8e19.png )

点击刷新，就可以看到这个新的 database 了：

![](./images/30efeb21da4468321ddeb981722e2d2d.png )
...
![](./images/f0b53082a2c166e3a2ca25cdd9c1d85a.png )
...
![](./images/10a52af3c656bcd465e578fb527afef8.png )

点击刷新，在 workbench 里也可以看到这个新建的表：

![](./images/639df18069bc5dcb1fbea2c403b85be4.png )

点击新建 sql，执行 select，也是可以看到插入的数据的：

![](./images/2f5d82352bc221639b8de33c57d2cdc7.png )
...
![](./images/891dc4cd0131f25e785445fe0fd2cd8c.png )

![](./images/c9f1c20adf815a0510e5286cabe4f9ec.png )
...
![](./images/36a54e73fce3dc6b557d0d3c10e64273.png )

重新 npm run start：

![](./images/2eff6b4df004b1ea218d2540aed431bb.png )

可以看到生成了这条建表 sql。

workbench 里也可以看到这个表：

![](./images/746ffe9530dbc78a5baf6e45c0635437.png )
...
![](./images/73b468d6e0f6f8657a2abbe3e54eea74.png )

在 IdCard 的 Entity 添加一个 user 列，指定它和 User 是 @OneToTone 一对一的关系。

还要指定 @JoinColum 也就是外键列在 IdCard 对应的表里维护：

![](./images/a2bff3b347bac070c368ff7109b8d0a3.png )

重新 npm run start：

![](./images/77bd46bcf477867f6f7484e99515e96d.png )
...
![](./images/4270129ca425ac3382a03d185d07e6a6.png )

![](./images/56146a1fcf6c0e9ae043a0bde2bb6fcf.png )

![](./images/34e16931ff6dae622122dc6bfeaa83c0.png )

生成的表都是对的。

但是这个级联关系还是默认的：

![](./images/6304edacbbd505bea33728ce3286aa0e.png )

如果我们想设置 CASCADE 应该怎么做呢？

在第二个参数指定：

![image.png](./images/0fa8703757918f51175a0c9d049dff31.png )
...
![](./images/08d200ab74c2ff2ecad45f922eeebe9e.png )

![](./images/f1052da7d870e8e75921e598598afe71.png )
...
![](./images/98f613a0651162903a6ec9b5cc55c6d9.png )

可以看到后面插入 id\_card 的时候，已经有 userId 可以填入了。

数据都插入成功了：

![](./images/1be607e9704c33bb4705df21b32354d0.png )

但是我还要分别保存 user 和 idCard，能不能自动按照关联关系来保存呢？

可以的，在 @OneToOne 那里指定 cascade 为 true：

![](./images/1d22a81220ef9b03fbe074c8186075c0.png )

这个 cascade 不是数据库的那个级联，而是告诉 typeorm 当你增删改一个 Entity 的时候，是否级联增删改它关联的 Entity。

这样我们就不用自己保存 user 了：

![](./images/13c3c49f3c2c4c17bec0f774ae78971d.png )

重新 npm run start：

![](./images/21dc6b16a0e8a4247d666afe2b0f3164.png )

可以看到它同样是先插入了 user，再插入了 id\_card，并且设置了正确的 userId。

![](./images/aa40731ba39654804fa3ad87a95fd0cd.png )
...
![](./images/0eee23ca818fd31190f93a5af8da1ad6.png )
...
![](./images/34498aa22f9e3349650c4dc1778031df.png )

再跑一下：

![](./images/e5b6f1969baa3220e173b811dda20d43.png )
...
![](./images/b60fe9b8a4784e46ae0e802215e68913.png )
...
![](./images/9d6fb60eaa553cde46bb1719b995abd2.png )

再来试下修改：

现在数据是这样的：

![](./images/8aab85d9cb86d368f3bf0b5bfc5e6643.png )

![](./images/db145abe3080fe2125e36878dac54e94.png )
...
![](./images/8e34c373f0bc181b174d76238b63d647.png )

![](./images/a8795e68dddc8d04a5683ae92c8b769e.png )

看下生成的 sql：

![](./images/150418edf2e692eaa614e840074ea132.png )
...
![](./images/6b0e160837a7144588d5a6278d45bb1b.png )
...
![](./images/c896d76ad11a41a67926f0e7e268a547.png )
