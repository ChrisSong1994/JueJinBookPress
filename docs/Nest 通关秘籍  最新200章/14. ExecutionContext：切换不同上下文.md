...
```
![](./images/e285785ff35a4d8b3c628fff15764b34.png )

然后创建一个 filter：
```
nest g filter aaa --flat --no-spec
```
![](./images/70be07e43ace409c7789f80d8cb8bd32.png )

![](./images/238514f6a1fb5f468f9aa44722d5d8cd.png )
...
![](./images/175654fa5995f1e366d9ad17359c978b.png )
...
![](./images/8e111954a9c1bfb68672784dd817afb6.png )

然后需要启用它：

![](./images/8e70885eb860c099e0e009611d034184.png )

路由级别启用 AaaFilter，并且在 handler 里抛了一个 AaaException 类型的异常。

也可以全局启用：

![](./images/37234adb1bb68b300ece454ab31b5975.png )
```javascript
...
![](./images/95f7a194c689b6b2d5d2474cca567339.png )

filter 的第一个参数就是异常对象，那第二个参数呢？

可以看到，它有这些方法：

![](./images/bdf84bfdd2fc8a6374b9596a87677aa7.png )

我们用调试的方式跑一下：

点击 create launch.json file 创建一个调试配置文件：

![](./images/6e00c07e69aa48e3be4d68d00e5fce8f.png )
...
![](./images/e75821b4f0650b4af3683d733d32d682.png )

打个断点：

![](./images/fe6cda9bc643db07a8d9428bdb1d178c.png )

浏览器访问 <http://localhost:3000> 就可以看到它断住了：

![](./images/7cae13caf3032ea1adf27305f8f5a273.png )

我们分别调用下这些方法试试：

在 debug console 输入 host，可以看到它有这些属性方法：

![](./images/1a449a67847188c3a50209e876d79bad.png )
...
![](./images/9de0d611fce15e526479ea5d34409b07.png )

当然，一般不会根据 index 来取，而且这样用：

![](./images/385c559115d8d5b62770cdbefd434da0.png )
...
![](./images/d25e6f575b5fdb6e85738d2a669ce7c7.png )

![](./images/95cc2e6a3e760629ab84bf0ffcaa4f99.png )
...
```
![](./images/3d094189c4be1508a60045b44890d12b.png )

可以看到它传入的是 ExecutionContext：

![](./images/5441ca00ac3426c2b8210ee9578a8bd6.png )

有这些方法：

![](./images/414e5ff5a1798da8a53e05f414a1fdb5.png )

是不是很眼熟？

没错，ExecutionContext 是 ArgumentHost 的子类，扩展了 getClass、getHandler 方法。

![](./images/d310168165e13e15714894cc28b73c6d.png )
...
![](./images/48aa9fa63d358242996fb7e37dd9035e.png )

在 Guard 里打个断点：

![](./images/48af41703659a7c0acad7febe3eacfd2.png )

调用下 context.getClass 和 getHandler：

![](./images/3b96a36e5a4da94c555223d3119b1cba.png )

![](./images/06860f22db50a40ef87edde7220bfab9.png )
...
![](./images/854b3a0ce565caa717ecbb2d809fa732.png )

然后定义这样一个装饰器：

![](./images/ab02251ae24cade9c17b0e66b8a39b2f.png )

它的作用是往修饰的目标上添加 roles 的 metadata。

然后在 handler 上添加这个装饰器，参数为 admin，也就是给这个 handler 添加了一个 roles 为 admin 的metadata。

![](./images/866961b92d021a79f38baa2dd36c2906.png )
...
![](./images/c2a1915df3ff8a847bacfffa1ca1cc74.png )
...
```
![](./images/e5ec1699d2644a11a6f3f5e4ce4f1504.png )

![](./images/13547f4d0e63d3275a7e74fedee3ddcf.png )
