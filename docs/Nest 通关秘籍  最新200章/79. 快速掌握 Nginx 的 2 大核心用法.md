...
![](./images/8fd3535b50130f8026f988ef673a31f5.png )

输入容器名和要映射的端口：

![](./images/fb4f4a2a82a49cc1e4103867052c8f75.png )

这里把宿主机的 81 端口映射到容器内的 80 端口，点击 run。

这时候就可以看到 docker 容器跑起来了，并且打印了日志：

![](./images/8c53d7020741109e8e3293162445a6c5.png )

浏览器访问下 http://localhost:81 可以看到 nginx 欢迎页面：

![](./images/519b7fb3f36cb64efa3ec085df0c0c9a.png )

这很明显是容器里跑的服务。

![](./images/d71fca10dfeb9bfdc08b29e2539d6dd7.png )
...
![](./images/245524b2475cb19c40fdd06fbbc8652e.png )

里面的 /usr/share/nginx/html/ 目录下面就是所有的静态文件。

双击点开 index.html 看看：

![](./images/5a5e7bc3580c192ce6121edae3692960.png )

![](./images/c31aa06783e2ca9de1701caf63798843.png )
...
```
![](./images/b989fe0af04e01990801be29a49910cf.png )

docker cp 这个命令就是用于在宿主机和容器之间复制文件和目录的。

![](./images/93b1813935f89bcbcdf761ae18ff6ff3.png )
...
![](./images/529e099e5f606421c99be350c9cc5147.png )
...
````
![](./images/10d0712128103695919ac8fc9d03d2a7.png )

但当目标目录存在的时候，docker 会把他复制到目标目录下面：

![](./images/6412c8b9e87e55587fb3b2e0b9e59cd0.png )

我们需要先删除容器的这个目录，再复制：

![](./images/f707eab7aa84ed861b329f6a7458cd4b.png )
...
![](./images/04dd5b23b4627e6b0ba2dfda4381195b.png )

然后浏览器访问下试试：

![](./images/bafa4f8eecafa98153eaf224e87306d2.png )

![](./images/1ced0b7c48b67359c5fc31225ca90502.png )
...
![](./images/a568ff4778e26a1b3666de12b59d0676.png )
...
![](./images/d0442ac4e2e2aa8470776fe86af570ed.png )
...
![](./images/50c38bc606507837f36a5542adf99a4a.png )
...
![](./images/7a78636c87ac2780c3e6c39c50243740.png )
...
![](./images/fec302916cd96aa97e2b1df929536c18.png )

然后来看第一条路由：

![](./images/5c3caf3aa6348c30990af9daf07da4a6.png )

location 和路径之间加了个 =，代表精准匹配，也就是只有完全相同的 url 才会匹配这个路由。

![](./images/2f14cf06ac460d06bef0b5045f599ec4.png )

![](./images/6a2fdcde7a638794b2de6dd90691a1fd.png )

不带 = 代表根据前缀匹配，后面可以是任意路径。

![](./images/a1a3eabeb009abb4cd1a762fb6a60a09.png )

这里的 $uri 是取当前路径。

![](./images/63ec3fc6b507876e8ae76719ad2fcb4a.png )

![](./images/94b36bbde1ade397b2309a61b8ee078d.png )

然后如果想支持正则，就可以加个 ~。

![](./images/d09401a7dd3e3cdfff925dc2c750af6a.png )

这里的正则语法不难看懂，就是 /333/bbb 开头，然后中间是任意字符，最后 .html 结尾的 url。

![](./images/9c180f0c40a389abb71873ec267029aa.png )

![](./images/26118500deab0117b6c55cda0977a92e.png )

但是它是区分大小写的，比如这样就不行了：

![](./images/d7d163711cea7f74cc515c5b2bf22405.png )

换成小写就可以：

![](./images/b68b303bd91c4b80af8742efa5831587.png )

如果想让正则不区分大小写，可以再加个 \*

![](./images/8862bc3a4c3ad6c551385573df4ae08e.png )

试一下：

![](./images/026e232806064572a88fc586f0b7755a.png )

![](./images/af14f2aa96119d3c5715353f645972aa.png )
...
![](./images/09f8b5dce58327906fcc6961578b1346.png )

这时候就有两个 /444 的路由了：

![](./images/95c2a949a3c4961c3f6028f81231c9e0.png )

这时候浏览器访问，还是匹配上面的那个路由：

![](./images/4c88e9a082fbd12dddb4b3ca3ae7e92f.png )
...
![](./images/ed71dcd2c527907d64f440f7656a1cde.png )

这时候同一个 url，匹配的就是下面的路由了：

![](./images/c74aacb36022964dd1d4000d681c7615.png )
...
![](./images/ed174701208ee2b8834339d5272d87de.png )

![](./images/1f344786c58e0c3a1f849d7b99bb798f.png )

![](./images/900798498359e021fea6173714efb6e6.png )

前面用过 root：

![](./images/21d7747c6a548775d6857b33a29e5b99.png )
...
![](./images/70dad84bbdf6b7a9a059ad7d4e9ba710.png )

![](./images/17901c2670fbabf5ed77b06f824f7d88.png )
...
```
![](./images/ecbd4dad4d8fea48d4c8e62916a72cc6.png )
...
npm run start:dev
```
![](./images/a722a4a5ba50ba06891547d37d8e3fcc.png )

浏览器就访问 http://localhost:3000 看到 hello world 就代表 nest 服务跑成功了：

![](./images/a04be282d3ab4b1b6b6427d60a5c7da5.png )

添加一个全局的前缀 /api

![](./images/aa648561848ecf1eb98251090f1f5af1.png )

![](./images/da29d6b597e95dfdf6ca52d508131453.png )
...
![](./images/6186ae2216853138cb3f41519d0c20fd.png )

也就是这样的：

![](./images/6137e23b66aa930af603ac13c909bf60.png )
...
![](./images/356b3b48fcb7cd1b187e3592cc999400.png )
...
![](./images/bdf552a9365be348892f36ceb0583ddb.png )

然后浏览器访问下。

直接访问 nest 服务的话，是没有这个 header 的：

![](./images/2d07c40ae714febe58cac8636391031b.png )

![](./images/4045d52edb89195b55ece0c254a8a16a.png )

访问 nginx 的反向代理服务器，做一次中转：

![](./images/a6a388487609f62c1d3f40dd61e98ef5.png )

![](./images/3ab34ca0cfc76c69475c69027cd31d99.png )
...
![](./images/53b661af79c4f23d5ab3f04ac475cfaf.png )

把 nest 服务停掉，然后重新 npm run start

![](./images/4790fe429233c71f2d7f7506b48975d7.png )

3001 和 3002 端口各跑一个：

![](./images/6681b6df785a36f78d4278bc46c677a3.png )

浏览器访问下，都是正常的：

![](./images/cf2ca13413902988bf7f992b60b4f1ec.png )

![](./images/f811ff3ac5d0ab78aa110a23a2ae6aca.png )

控制台也打印了访问日志：

![](./images/ebb48765540c4f345eedac8b2e3fd54d.png )

问题来了，现在有一个 nginx 服务器，两个 nest 服务器了，nginx 该如何应对呢？

![](./images/8bcd7df3744d0c11f313eb90289229f8.png )

nginx 的解决方式就是负载均衡，把请求按照一定的规则分到不同的服务器。

改下 nginx 配置文件：

![](./images/e109cf00de7d9c6d4aeaf49808f74baa.png )
...
![](./images/8a7ce31bc678e2dcb990472a49c5b70a.png )

可以看到两个 nest 服务，一个 3 次，一个 2 次。

![](./images/29cf1b3eee98d46ce420f5522259cccb.png )
...
![](./images/427940098782bd8238f8f92c28ed47ec.png )
...
![](./images/4d43febdd7e01287dafb2d3b30898432.png )

然后我访问了 8 次 http://localhost:81/api

看打印的日志来看，差不多就是 2:1 的轮询几率。

![](./images/dfeffd9b1b73d5d111c94a94cd946c24.png )

这就是带权重的轮询。

我们再试下 ip_hash 的方式；

![](./images/9272c4c03a504547f24ff1bea801d389.png )
...
![](./images/caffc036b7e86c453a74bd7ff5187efa.png )
