...
![](./images/29fe5becc849de16ded8dfa47b0b9fd1.png )
...
![](./images/d996300ef1113099e267b3df2a04ea03.png )
...
![](./images/1d0097354040eff0f672e8c44dae6f87.png )

因为 index.ts 里用到了 User，我们用不到，把这些代码删掉：

![](./images/770b3c11d7b7ef4815c9637d3bd7e335.png )

然后 npm run start：

![](./images/556fc0c81884f74ee9dd7e559c3ebc73.png )
...
![](./images/ed14e60913a0d97cd425e85c193a053c.png )

在多的一方使用 @ManyToOne 装饰器。

把这两个表删掉：

![](./images/86e2d833bba8de26a1c7efd814937ef0.png )

重新 npm run start：

![](./images/e8ae99197d435fc96b26f8ae40b0b662.png )

就可以看到创建了两个表，并且在 employee 表添加了外建约束。

workbench 里也可以看到这个外键：

![](./images/5bdff8297df5aa80e62f517abcd81764.png )
...
![](./images/4151aa8eab6045c3e25c100d061b47bd.png )

可以看到被 transaction 包裹的 4 条 insert 语句，分别插入了 Department 和 3 个 Employee。

当然，如果是设置了 cascade，那就只需要保存 empolyee 就好了：

![](./images/121e6cbce600f0ef58387f49b00c2127.png )

![](./images/f3a0c82e7f3559c3cb8f6a7fd5d35f43.png )

department 会自动级联保存。

不过一对多关系更多还是在一的那一方来保持关系，我们改下 Department：

![](./images/a80829f1d1a75640f4350d0b54c1e294.png )
...
![](./images/4179fee846391c03c722cd00e63e4c44.png )

加上 @OneToMany 装饰器，再设置下 cascade：

![](./images/6bbcb48acd18ca386bcd3e88bbc65f36.png )

这样当你保存 department 的时候，关联的 employee 也都会保存了。

不过这时候要把 @ManyToOne 的 cascade 去掉。

![](./images/e400fc0859f237b2b4dd1fb2a57f821c.png )
...
![](./images/64ec7f75861145730785026562b8e8ef.png )
...
![](./images/f7df9a5c682bfec819a20d2b6f056a5c.png )
...
![](./images/79cffa40a025f25535d642b6707bb6b9.png )
...
![](./images/b99a8d12de526e4c0c34948057f01163.png )
...
![](./images/1688a00e4e0b9d178512de32a12b3be0.png )

那就不用自己删 employee 了，只要删了 department，mysql 会自动把关联的 employee 记录删除，或者是把它们的外键 id 置为空。

![](./images/4cf9fdb045bd33f446504de97c72c619.png )
