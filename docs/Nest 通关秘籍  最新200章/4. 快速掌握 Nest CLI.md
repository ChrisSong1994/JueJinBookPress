...
![](./images/c92c85b330e58abb0a615fa79ba153e9.png )
...
![](./images/67af8b42b84af755047b449e6542f86c.png )

--skip-git 和 --skip-install 很容易理解，就是跳过 git 的初始化，跳过 npm install。

--package-manager 是指定包管理器的，之前创建项目的时候会让我们选择：

![](./images/752dc8999d92a58284bffc4606d9bb36.png )

指定之后，就跳过包管理器选择这步了：

![](./images/ae0cdd29cefa41f5d613a573afc70195.png )

--language 可以指定 typescript 和 javascript，一般我们都选择 ts，用默认的就好。

--strict 是指定 ts 的编译选项是否开启严格模式的，也就是这么 5 个选项：

![](./images/1f2ea3c008575258426a87be97ce4ce0.png )

默认是 false，也可以指定为 true：

![](./images/5b641e8cf444b42649dce09beed418be.png )
...
![](./images/1b0cc5d29457e991898f35f9c722e55b.png )

它会生成 module 的代码：

![](./images/889ae51035ecc7def391cee315e758c6.png )

还会自动在 AppModule 里引入：

![](./images/db3198ef2e5167bf7de229e094b42c48.png )
...
```
![](./images/95a79137c95ed35ad6b6977cef7ca7fb.png )

同样，它也会更新到 module 的依赖里去。

生成 service 也是一样：

![](./images/63967929cc7a2e9857181888202602c6.png )
...
![](./images/887640ff90def14ea449870404b43485.png )

然后会让你选择是否生成 CRUD 代码：

![](./images/b400f46ade47e3eb30106a79e5000d7a.png )

选择是。

然后就会生成整个模块的 CRUD + REST api 的代码：

![](./images/9f1bd92cf39083d2e5f5178e48754a6d.png )

![](./images/0ec26f725cb42bec21cc2b36e7bc8dbf.png )

当然，它同样会自动在 AppModule 引入：

![](./images/d1bafff9e37dcfd0e084f727fc894639.png )

这就是 nest generate，可以快速生成各种代码：

![](./images/ca67ca313ba25167dac8304d4ba0162c.png )
...
![](./images/520dfbc48390d8449dffa2b25d9f449f.png )

它的实现原理很简单，就是模版引擎填充变量，打印成代码：

![](./images/c5ed940e3612461195414a66ecdb229c.png )

nest generate 也有不少选项：

![](./images/498970d768abe2c6076835c0d18d904b.png )

--flat 和 --no-flat 是指定是否生成对应目录的：

![](./images/918e3601092d051748f268847ac98aa0.png )

--spec 和 --no-spec 是指定是否生成测试文件：

![](./images/2edcd3e2fddb53478c6f223fa7776e48.png )

--skip-import 是指定不在 AppModule 里引入：

![](./images/57a0ec4773a181fd57104e2f3df44607.png )

也就是不生成这部分代码：

![](./images/d316e07107742f6416c29be31fad8d6d.png )
...
![](./images/50b32793349cea7afb47879ca968a67d.png )

执行 nest build，会在 dist 目录下生成编译后的代码。

同样，它也有一些选项：

![](./images/6d72d2029f7a21b44b05adb4ccaadade.png )

--wepback 和 --tsc 是指定用什么编译，默认是 tsc 编译，也可以切换成 webpack。

这是 tsc 的编译产物：

![](./images/9fee30ed07ffa33472aede06900924e6.png )

这是 webpack 的编译产物：

![](./images/50f9d0288c7840b7f796706a5893d702.png )
...
![](./images/c93cc6a1a9d77d15407356c26f77701e.png )

比如 compilerOptions 里设置 webpack 为 true 就相当于 nest build --webpack，一样的效果：

![](./images/1ab97f620bf95568f2464dc0ea0cac6b.png )
...
![](./images/180208d211a8fb6c861510279a070644.png )
...
![](./images/f77138da1b80aa12d9d1780cf9378b13.png )

可以看到每次重新 build 了，并且用 node 把 main.js 跑了起来。

它有这些选项：

![](./images/bdde6069000b74fce4fff5107853af82.png )

--watch 是最常用的选项了，也就是改动文件之后自动重新 build：

![](./images/5f33b1b4a972df5f6368d18a78e1fb1a.png )

--debug 是启动调试的 websocket 服务，用来 debug。

![](./images/42db0b5e7b369db2dda9e97c49720bfe.png )
...
![](./images/f092018ea5c7b0b47b078f96cb80994c.png )
