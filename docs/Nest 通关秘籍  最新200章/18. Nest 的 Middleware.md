...
![](./images/f7958c3b93fb2296c1b961b496c2146b.png )

因为这时候并不知道你用的 express 还是 fastify，所以 request、response 是 any，手动标注下类型就好了：

![](./images/488f3b0547788c17d50ad025bcae92da.png )
...
![](./images/345da918a8ef2daf1959790c18468cd4.png )
...
![](./images/f48d6254882a5ae7f548277fbbd665cf.png )

可以看到中间件的逻辑都执行了：

![](./images/88b1f17d52679557e0b49b900645d334.png )

这里也可以指定更精确的路由。

我们添加几个 handler：

![](./images/7effc972de9f0731c83f0ac7e9e39295.png )

然后重新指定 Middleware 应用的路由：

![](./images/f5e04536ca704a078d442ce45fde24fc.png )
...
![](./images/0a18f1c583a141026804473903305c63.png )
...
![](./images/e0de5063334b90bc6cc26e0acb993040.png )

当然，这里也可以用构造器注入，这样更简洁一点：

![](./images/af02c76ec8607a320c4e7dc2d1812a02.png )

这时在访问这个路由的时候，就可以看到中间件成功调用了 AppService：

![](./images/d269732e0a9a58a3dbfe916134bf533b.png )
...
![](./images/27b619f333c6cf55a1ae7f8b31b2f460.png )
...
![](./images/1bda5831fdf1367a28e9c92c490b3be0.png )

不过这种形式不能注入依赖，而且也不能配置应用到什么路由，不建议用。

app.use 等同于在 AppModule 的 configure 方法里的 forRoutes('\*')

![](./images/6754527f78c5a4d810e37ebb56ecce15.png )
...
![](./images/e86fb3c4fb053ccb9ec3b66723c114ed.png )

但如果是这样一个 handler，它就不返回值了：

![](./images/3793b067bb050969bbda1ff495f4929b.png )
...
![](./images/adc294c5063e45aa0258f42dbc756ce2.png )

这个上节讲过。

当然，传入 Next 参数的时候，一般是不需要在这里响应的，一般是调用下个 handler 来响应：

![](./images/782c18dc8eafa61d6509ef889ed7acfa.png )

![](./images/31c6600f5dbcb13928b7ff5e1e32f7eb.png )
...
![](./images/0eb4b74f17bd8f14878c669b400d0f74.png )
...
![](./images/e586bf8c4093d99edad3de449c56744b.png )
