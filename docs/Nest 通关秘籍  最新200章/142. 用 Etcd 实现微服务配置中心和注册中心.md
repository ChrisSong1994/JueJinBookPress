...
![](./images/25838b042c22db2ee648f67389ff82e8.png )
...
![](./images/49d526e2186be4eb688aae1125e07550.png )
...
![](./images/72338258183c8f9fb5b0c392c108008c.png )

答案也是需要一个单独的服务来管理，这个服务就是注册中心：

![](./images/f2182916a0a03c62ec2c0684c34ab884.png )
...
这个：
![](./images/6bfcdc0d8c1cd14e1f0e59cd5ade784e.png )

这个：
![](./images/3a0c8d6a403cd2da8d37bbcb25921f4e.png )

或者这个：
![](./images/0fdf91a94f8bfc9eb87afb473f5d7b29.png )
...
![](./images/94bab941384dc53832efb9392cd125c7.png )

它其实是一个 key-value 的存储服务。

k8s 就是用它来做的注册中心、配置中心：

![](./images/730fee6ac941d5065b343a928d6f9e15.png )

我们通过 docker 把它跑起来。

在 docker desktop 搜索 etcd 的镜像，点击 run:

![](./images/12cb70b3dd9c65815643d63877b3e588.png )

![](./images/678921c9dd0e52775f5ba99bf50baa74.png )

输入容器名，映射 2379 端口到容器内的 2379 端口，设置 ETCD_ROOT_PASSWORD 环境变量，也就是指定 root 的密码。

然后就可以看到 etcd server 的 docker 镜像成功跑起来了：

![](./images/a4f93d9acd903baea1474605f34b45b0.png )
...
![](./images/4889cd731da5260d205aefbf9a369861.png )
...
```
![](./images/e95fa038de457b13dc9c692f349ff419.png )
...
```
![](./images/3e6da6b4f068609b33a72cfbbe6658ac.png )
...
![](./images/6ebb3e6f56a4a075e8dabef922e0adc2.png )
...
![](./images/60c133f95e5b6e1cade2b6cfb14cbd08.png )
...
![](./images/02c10b7fddf0de126e33128fe7ab326b.png )
...
![](./images/b26d9424cdf2b47ab754833ddd6a723c.png )
...
![](./images/fb176f10d09fb999e8eed9534f9a4002.png )

确实取到了 etcd server 中的值。

然后在 etcdctl 里 put 修改下 /services/a 的值：

![](./images/2ae73a7ff230182cffaaca78d95340a4.png )
...
![](./images/cba3bcdcd2535d7aa744ecc792689bdd.png )

再 del 试下：

![](./images/9b3606c655731fadbf3080536aafa72c.png )
```
...
![](./images/6eb3ae4ca3211bc8acf6b3bffd556060.png )
...
![](./images/dfc95af259a08492ab83e69e1bac5ffc.png )
...
![](./images/9330a17c56ec394f358e23edf63b4ef5.png )

当在 etcdctl 里 del 一个服务节点的时候，这里也能收到通知：

![](./images/ace0b1ff3c26cfde083dd804654d689f.png )
```
etcdctl del /services/my_service/instance_2
```

![](./images/e77aaf47a7bb69f6645d8cfc0de7380e.png )
