...
![](./images/8f2a3da1d53d513f0d31339e870c6541.png )
...
![](./images/bf772562b054c1e5c4036d10dee13bd4.png )

synchronize 是根据同步建表，也就是当 database 里没有和 Entity 对应的表的时候，会自动生成建表 sql 语句并执行。

![](./images/496857f933d1b4ff7d5b7eddf153f76c.png )
...
![](./images/609476bdbd7edba78d76ff9fd43b47b5.png )

而创建表的依据就是 Entity：

![](./images/7695271377fe55b5a5935a3649fa5e21.png )
...
![](./images/49369b3eb16e3392a5c22cda540adac1.png )
...
![](./images/ea1b4e78872e8249e8dec969e10778f6.png )

然后在 DataSource 的 entities 里引入下：

![](./images/2a47249b79be56ac0b533c61433a2e40.png )

重新跑 npm run start。

生成建表 sql 是这样的：

![](./images/2f07a42bc0a1780c7e49702b32951a67.png )

格式化一下：

![](./images/c7ad0e83daa6dc92aa4c3f83d5534b9c.png )

对比着 Entity 看下：

![](./images/35a569cadbc0bc661368b98913101b6f.png )

是不是就明白怎么映射了？

在 mysql workbench 里看下，确实生成了这个表：

![](./images/350e8e606599fa89783e712d95c4e4f1.png )
...
![](./images/bc294995a8ac56e309065f9823187286.png )

![](./images/f9e0a833515867e9dfa0f1f1031cbada.png )

可以看到数据库插入了这条记录：

![](./images/d09e9bc370ce66b20ed76f86d71b6f7e.png )
...
![](./images/326d0493cb37650efd1ac738f76bd354.png )

可以看到，生成的 sql 语句变成了 select 和 update：

![](./images/6ef40218f3b0c8e1cee372f124527bc7.png )

当你指定了 id 的时候，typeorm 会先查询这个 id 的记录，如果查到了，那就执行 update。

在 mysql workbench 里看下：

![](./images/372631cf82742f1d1bd66fea1567e347.png )
...
![](./images/fc7b83c577abba5f60fb46b8219e2f4b.png )

可以看到确实生成了 3 条 insert into 的 sql 语句。

数据库中也能看到：

![](./images/24ce15f01b127cab3e9dcb502334b086.png )
...
![](./images/bc0fdc54c0f995e522ecbb392b7b81d2.png )

在 workbench 里也可以看到数据被修改了：

![](./images/fecf4eeba54ee4b3d20c4b02e7599406.png )
...
![](./images/8f6fc8e303bda161b0c9a18e885441b3.png )

数据库了对应记录就被删除了：

![](./images/159d9991ccd7e1600c3a43c68e28ea2d.png )
...
![](./images/974e3f2d5d0e27d16fcc0d763510d6fd.png )
...
![](./images/2e8c1dfdb261ec4a0325b322f8a95dda.png )
...
![](./images/23165649b18c3cd5081df495d256aed4.png )
...
![](./images/0a3d01f5ebc67f9f005fc2dd20167b83.png )
...
![](./images/a48a6ef1ea7966eaa64c22755b021448.png )
...
![](./images/472a9cd1f6cdb1c148893dfbc060feee.png )
...
![](./images/587a82bb9d2655f4dbf4b37b92cf5f0a.png )
...
![](./images/992a7663706cae5ca4deab9be83858af.png )
...
![](./images/68557e57d7b1a50d97c0465da2d675f1.png )
...
![](./images/6b5591972ed0dd7cbcabb12e53b59ff3.png )

有同学说，用 query builder 和我用 find 指定 where 有什么区别么？

比如这种复杂的关联查询：

![](./images/4768cd6fe4d5042d04f4827011a24eeb.png )
...
![](./images/f618bc62d3e37c2d3c3d43d14ae7673a.png )

还有，调用每个方法的时候都要先传入实体类，这也太麻烦了：

![](./images/1e634f2425c2cff8bddefb3bcb941eb1.png )

有没有什么简便方法呢？

有，可以先调用 getRepository 传入 Entity，拿到专门处理这个 Entity 的增删改查的类，再调用这些方法：

![](./images/26061224c8b7bc9f7f70c2eeee927f81.png )
...
![](./images/4e228820d57cc0be9ed05536615c16a3.png )
