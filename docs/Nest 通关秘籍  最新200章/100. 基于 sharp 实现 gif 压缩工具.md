...
![](./images/09c77569b256dbef6ed792c16d16dbcd.png )

在编辑器上传会提示超过 10 M 了：

![](./images/cf0f0b892c252722fae5915339f17156.png )

这时候就需要 GIF 压缩，不然文章发不了。

于是我在百度搜素 GIF 压缩，就找到了一个工具：

![](./images/6fd302e80b92f0700da56d2714fdc117.png )

它确实能解决我的问题：

![](./images/b03a28c5bd72308db74fe31cc5db6ab6.png )

但是要花钱：

![](./images/7764ebd9cfceb78e4df61cfd2aa82f80.png )
...
![](./images/81a61dc0571a0464fe606f36d5d767f6.png )

但最近发现有 npm 包可以做这个，没必要买这种网站的会员。。。

当时我的心情是这样的：

![](./images/3ba10db9b318f42c291ecb5e68f88df2.png )

这个 npm 包就是 sharp，它是用来处理各种图片的：

![](./images/5b1c6daf426fb5f0027ce8754eda97b6.png )

![](./images/fdb0a73a2b820762940f287592a75215.png )
...
![](./images/0749b8b6fd0c40be8ae8b814118b6f2c.png )

要知道之前的 1.image.gif 可是有 21M 啊：

![](./images/03425a262dfddc0c674f0740ed466828.png )

然后打开它看看：

![](./images/ed12083c87f8712d2f77e8c5befee8f6.png )

没啥问题。

回过头来，我们再来看看这段代码：

![](./images/4c0540971d7eb287f5592500be550d4d.png )
...
![](./images/fd0c1dfd80ea19c0f76f04a6a03fb80f.png )
...
![](./images/c1ffe694531ed01ef9b6e61eeb3ea19f.png )

图片也更小了一些：

![](./images/afe36db447580d9dada71c1cee977bd2.png )
...
![](./images/7fd7721d699ea70266b9251662d513d5.png )
...
![](./images/c93f30c011ad29ddc9581ee19d000db7.png )
...
![](./images/16a643d643f470107389b049e9bd1cba.png )

antd 会 post 方式请求 action 对应的接口，带上上传的文件：

![](./images/fa876ac3c58fb2d3fc87fff1eec86973.png )
...
![](./images/3e14755965958bd2c52372bf338f24fc.png )

修改 main.ts，启用跨域支持，并修改启动端口为 3005

![](./images/6faef18e1661e820a3ef0dc808b09503.png )
...
![](./images/52da5332e2e2141893e5397ba04c2448.png )

浏览器访问 http://localhost:3005 可以看到 hello world，说明 nest 服务跑成功了：

![](./images/c17b773738d3399e1e4500de569ea60b.png )
...
![](./images/1f2ec7b75d487926c763146baf2f1419.png )
...
![](./images/a29fad5c972dbc0b7f559b91ac1c309a.png )

测试下上传：

![](./images/12444d8fa4d1e569dbd6127dc50548e1.png )

提示文件上传成功了，然后在服务端控制台也打印了文件信息，并且在 uploads 目录下可以看到这个文件：

![](./images/e23f8393f9fa40ec40481bdc5aa261a1.png )

这些文件在浏览器打开，可以看到就是上传的 gif：

![](./images/db6e69aa340fc919852cf80a14da5760.png )

然后我们把文件路径返回就好了：

![](./images/460c271d1372cbf9a59e7ca097c1ed89.png )

现在上传文件成功之后就可以拿到这个文件在服务端的路径了：

![](./images/5b72a134f98a2ba5cc6d67197fa86b5a.png )
...
![](./images/390eb93af94f75d5b2d574a5b89598a2.png )

服务端接收到了传过来的参数：

![](./images/ce769623aa00cee1622794ff1e31a5a2.png )

然后换一个真实存在的路径，返回 success：

![](./images/b68e0127c44ad7dcd56c7842ed158c1a.png )

说明服务端找到了这个路径的文件：

![](./images/6592b90777072e26f628bd044d4d169d.png )
...
![](./images/92088b591ef9d5d7f7bd80e15c1b00a8.png )
...
![](./images/1f7f48e8ca8aff43bb887b77dfcf73bc.png )

访问这个接口，带上文件路径和压缩的参数，会返回压缩后的文件：

![](./images/db8050d6e7d0d3aa95e06c4f89c88284.png )

然后我们在前端页面上加一个表单来填参数，然后访问这个接口压缩文件：

![](./images/e82165649bc0ce6b3b5a4dc12e87ff6e.png )
...
![](./images/66da512cad3d9c520b090b750eb56274.png )
...
![](./images/5800159a2b75a710bdf9f453af14f409.png )
...
![](./images/50d19d47a7588406cf725633962fcfcc.png )
...
![](./images/cc6d0a8e75b99f74f96db67092b23009.png )
