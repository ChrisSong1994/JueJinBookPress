...
![](./images/55a5665b87a8a19212f2ed9115bfbd52.webp )
...
![](./images/8ee41601c7df501543658fcfdbd6df9a.webp )

要注意的是 `Headers` 中要带上 `token` 信息如下：

![](./images/7177fe6c0c812ab777bdfa0ee6e1b6dc.webp )

此时我们查看数据库内是否已经添加了数据，如下所示：

![](./images/29dafc616412970e2b80859bdc607b4f.webp )
...
![](./images/d5e5975a2ceea58dafa41fe3b356cb10.webp )
...
![](./images/82a25767ccd671eae098709ba8574895.webp )
...
![](./images/90ad2a3cdb94a1e50f7589411f6f27dd.webp )
...
![](./images/90749abc3e4bc7d4aa41c8f292df5e72.webp )
...
![](./images/93ae70cf295c935e292d4b7bdd6bddd8.webp )

通过详情接口，请求是否修改成功：

![](./images/7c14c342df4dc2a1afd2ffadee65d795.webp )
...
![](./images/33ca0d5cb8d91efb8a7654ac21ed40dc.webp )

报错信息为 “token 已过期，请重新登录”。这说明我们之前生成 `token` 时，配置的时效生效了。

![](./images/acf777fad762fb50215c80a19baaeedf.webp )

当然，你可以将有效期设置成 1 分钟，这样方便测试有效期是否生效。

我们重新通过登录接口获取新的 `token`，如下所示：

![](./images/09fa0cc1949ae6abbbfc62931c26cedc.webp )

通过新的 `token` 再次发起删除接口请求：

![](./images/91a5c9b630ea92a117182a41e9b20ef3.webp )

![](./images/f9cb03390c4ace0f1159595de5d0c326.webp )
...
![](./images/79997f521ed85c20b2bca875c393dc57.webp )

首先是头部的汇总数据，并且接口支持事件筛选，以 `月` 为单位。

![](./images/e49980b2c484c5234610785a8895697e.webp )

其次是收支的构成图，对每一个类型的支出和收入进行累加，最后通过计算占比以此从大到小排布。如上图所示，当前月份的所有学习支出是 `2553`，这个累加计算，我们在服务端完成。

![](./images/b83ef0b0a5fe1009396f70d91c4c3b03.webp )
...
![](./images/b0b64c3816bd47d348628fa3383cb6bb.webp )
