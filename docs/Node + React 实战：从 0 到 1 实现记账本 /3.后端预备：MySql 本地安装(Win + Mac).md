...
![](./images/9165440ef0cd4d1896fa4a8011eeb57e~tplv-k3u1fbpfcp-zoom-1.image.png)
...
![](./images/e1dab5931e824ff2815bc5b72b28b57e~tplv-k3u1fbpfcp-zoom-1.image.png)

点击「Download」之后，会让你注册登录账号，此时可以选择点击下面这段文字，跳过注册登录。

![](./images/77d5a83c24554b299bc6cad6c3635bd2~tplv-k3u1fbpfcp-zoom-1.image.png)

**解压**

下载完成之后，解压到自己想要放置的目录下，比如我就将其解压到我的 `C:\mysql` 目录下，如下所示：

![](./images/97ddc907b5854ace9954c6b84a94cb82~tplv-k3u1fbpfcp-zoom-1.image.png)

> 注意：此时解压后的文件夹中没有 data 目录和 ini 文件。

此时在 `mysql-8.0.24-winx64` 文件夹内新建一个空的 `my.ini`。如下：

![](./images/45fbfeb1799743efbba108f4798cd434~tplv-k3u1fbpfcp-zoom-1.image.png)

**环境变量配置**

打开控制面板，点击「系统和安全」，进入「系统」点击高级系统设置，如下所示：

![](./images/55b58583af264cf290b42f69657fe5ee~tplv-k3u1fbpfcp-zoom-1.image.png)

点击右下角的「环境变量(N)」按钮，在系统变量里新建名为 「MYSQL_HOME」，变量值就是你上一步解压后存放 `MySQL` 文件的安装路径。我的安装路径如下所示：

![](./images/5d10088b081e458eb0f51d5e7c7bf893~tplv-k3u1fbpfcp-zoom-1.image.png)

设置 `Path`，在 `Path` 里面新增 `bin` 目录。双击 `Path`，然后点击新建按钮，添加 `%MYSQL_HOME%\bin` 如下：

![](./images/2051620633140_.pic_hd.jpg.png)

此时你再观察全路径，`MYSQL_HOME` 已经被解析成具体的路径，如下所示：

![](./images/fb960c3827ed4e449b69bf0db21d06b3~tplv-k3u1fbpfcp-zoom-1.image.png)
...
![](./images/7be74f5c595d4070b1d5620128866f01~tplv-k3u1fbpfcp-zoom-1.image.png)
...
![](./images/b9d3d686167842f6953a366a21346666~tplv-k3u1fbpfcp-zoom-1.image.png)
...
![](./images/cc0fb5c438f14718884bb1bf7f7d91db~tplv-k3u1fbpfcp-zoom-1.image.png)
...
![](./images/d4141da8bffe455c9d0d6dc272166436~tplv-k3u1fbpfcp-zoom-1.image.png)
...
![](./images/b399bc6352ee4db695ed48f37a3315bc~tplv-k3u1fbpfcp-zoom-1.image.png)
...
![](./images/2f95b266390d4fb1851a4eae55608ff3~tplv-k3u1fbpfcp-zoom-1.image.png)

> 注意，如果安装完之后，出现一些小问题，如无法载入的情况，建议点击 「Archives」选择稍微低一些的版本，如 8.0.20 等。

这里我选择的就是 8.0.20 的版本，如下所示：

![](./images/d06d63be602248079641f724c37779d6~tplv-k3u1fbpfcp-zoom-1.image.png)

下载完成之后，点击安装，按照步骤往下走，直到需要密码的适合，一定要记住自己设置的初始密码。

![](./images/d39ca1d7110142a195509a53e3562ff9~tplv-k3u1fbpfcp-zoom-1.image.png)

后续链接数据库，需要这里设置的密码。

点击 「Finish」之后，我们点开「系统偏好」启动服务：

![](./images/51a475367ad3413c9af285bb686bb14b~tplv-k3u1fbpfcp-zoom-1.image.png)

![](./images/7f3335f0ca814b19b0e6303b51bf9aee~tplv-k3u1fbpfcp-zoom-1.image.png)

此时我们在命令行输入 `mysql -u root -p` 命令，会提示「commod not found」，我们还需要将 `mysql` 加入系统环境变量。

1、进入 `/usr/local/mysql/bin`，查看此目录下是否有 `mysql`，如下所示：

![](./images/0a22ec1a816f4174851d9cb97b2e0839~tplv-k3u1fbpfcp-zoom-1.image.png)
...
![](./images/0ae7412f25a4494abf8e294a744a30b5~tplv-k3u1fbpfcp-zoom-1.image.png)
...
![](./images/e1b563726de846d7af037f5d8f019bb0~tplv-k3u1fbpfcp-zoom-1.image.png)
...
![](./images/9c32953d369749a1acdd1c0917624464~tplv-k3u1fbpfcp-zoom-1.image.png)
