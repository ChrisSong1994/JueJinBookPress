...
![](./images/eddefd48b7acdc2d817c08d1e6a9ccb8.webp )

![](./images/c4e5352b2b12ef4d34037ba0f7d711c4.webp )
...
![](./images/e901117855935161beb39ed4b8d5c751.webp )

![](./images/60e3a3d309a901061d6be4ad7b0e772f.webp )

因为这些只是声明类型，而没有具体的 JS 实现，TS 就给单独设计了一种文件类型，也就是 d.ts， d 是 declare 的意思。

比如 lib.dom.d.ts 里的类型声明：

![](./images/c1d6abd18b01c2ad693a85ac6a797d3a.webp )

因为是 ts 内置的，所以配置一下就可以用了：

![](./images/59418c8129d550d5695d66d9c873b8c6.webp )
...
![](./images/837806e0036b71c040020151649ef3a7.webp )

TS 会先加载内置的 lib 的类型声明，然后再去查找 @types 包下的类型声明。

这样，其他环境的类型声明就可以通过这种方式来扩展。

![](./images/cd9bd7fc8600a94f6787576e581b0c6d.webp )
...
![](./images/36c86839944aa1c7a4732e2a176382cf.webp )

一般来说，很快就可以发到 npm 的：

![](./images/3e1beee2341f90f19cb0e0aad44b330d.webp )

我们知道，TS 内置的那些 lib 是可以配置的，扩展的这些 @types/xx 的包自然也可以配置：

![](./images/d75d36225dbc9d418db52eca3e30200f.webp )

可以指定加载 @types 目录下的哪些包，还可以修改查找 @types 包的目录（默认是 node_modules/@types)：

![](./images/582c2e54b76620c343ca8b8af8ccc097.webp )
...
![](./images/c521d1de1a0bfe62e65b7ab299076a19.webp )

然后在 package.json 里配置 types 来指定 dts 的位置：

![](./images/20a8bc0cc55fa061eee7b8ed5a751cfe.webp )
...
![](./images/3918983cbf7b0bf6dc3e968ab1f6c43a.webp )
...
![](./images/3ee15e6e3854731126e3c13362151a8f.webp )
...
![](./images/7961405da6814bf2aac08dfdb3c86fb1.webp )
...
![](./images/0082db5c9993182ab77535a65dbbf77b.webp )

![](./images/23a2a62b5cb9e92b050308f22c30aadd.webp )
...
![](./images/e5594f74aa355feb24426b47b7d15984.webp )

在 global.d.ts 里声明一个 func 函数：

![](./images/689e65ffcf8e66acefc42466a31054ee.webp )

在 src/index.ts 里是有提示的：

![](./images/b13e9405d32993b159b65272c850e02b.webp )

编译也不报错：

![](./images/fb214494afe7bb9d8cfe9eee5b6a992a.webp )

加上一个 import 语句：

![](./images/a1ab0d40bee0c96f7b98d3591bbb67b1.webp )

编译就报错了，说是找不到 func：
![](./images/d299dcad15a92254063c7fe88cf449e9.webp )

这说明 func 就不再是全局的类型了。

这时候可以手动 declare global：

![](./images/d9b15c2af903d01a84157dc921b937b6.webp )

再试一下，编译就通过了：

![](./images/c86c6a4bb5c6966196bf000a4936d125.webp )

而且不止是 es module 的模块里可以用 global 声明全局类型，module 的方式声明的 CommonJS 模块也是可以的：

比如 @types/node 里就有不少这种全局类型声明：

![](./images/c04d3ad771bba683816433b511781114.webp )
...
![](./images/4e8e4bc0bbf42c4c432a8d0e4d65715e.webp )

可以看到很多 dts 都这样引入别的 dts 的，就是为了保证引入的类型声明依然是全局的：

![](./images/86444e4d2627addafafc70f1cdc7f620.webp )
