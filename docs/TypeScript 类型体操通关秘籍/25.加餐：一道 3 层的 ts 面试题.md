...
![](./images/c4674e125ab3bc28a19708be341790d4.webp )
...
![](./images/ecc540d3e454d26827589901c3dd2755.webp )

![](./images/9c11efb16b905889cdeded15a11db212.webp )
...
![](./images/d0f2f6c7864a62505d3bdf42ebd7e836.webp )

但是 any 也可以赋值给任何类型，但 unknown 不行。

![](./images/b3278b101a44e97a60a57acef2c6ad85.webp )

![](./images/f96bb360fb123700268625a00a291245.webp )
...
![](./images/1807ba291772f40a0d8bbaec69957c73.webp )
...
![](./images/6d713ef9ae359e833a5c3667f7130da7.webp )

![](./images/9f1af50cb2b120d022862853d2b0b36b.webp )
...
![](./images/9d1167ac9cc449d05c05c1b1bed91141.webp )

但你把它作为返回值加到函数上会报错：

![](./images/9b7eea3f29423933033b82c44087444f.webp )

因为声明函数的时候都不知道参数是啥，自然计算不出 Zip<Target, Source> 的值，所以这里会类型不匹配：

![](./images/5f9b3f85f666c5112b4672d117175280.webp )

那怎么办呢？

可以用函数重载解决：

![](./images/82cd9379bdc3684c9d313ebebc54fa1f.webp )
...
![](./images/fdf06104233ad4ade7e7c586a9ae04f2.webp )

咋返回值的类型不对呢？

![](./images/1b28840b2a7e18658f7b17df81f1c8ff.webp )

其实这时候匹配的函数类型是对的，只不过推导出的不是字面量类型。

这时候可以加个 as const。

![](./images/4fcb39e62f0fbeb1c69d5c4cfb84a91a.webp )

但是加上 as const 会推导出 readonly [1,2,3]

![](./images/c36780c349d43544387a890235713ce2.webp )

这样类型就不匹配了，所以要在类型参数的声明上也加上 readonly:

![](./images/0a3368b8e96a5c0e3512c3124acfe4cd.webp )
...
![](./images/cd2e7d32dec671c780691798901adbdc.webp )

可以把索引类型的每个索引都加上 readonly 修饰：

![](./images/88c701b4c749f401b49b121afedecc27.webp )

但没有提供去掉 readonly 修饰的高级类型，我们可以自己实现一下：

![](./images/434b68fca1003978771f5024fb0ee8f5.webp )

用映射类型的语法构造个新索引类型，加上个 -readonly 就是去掉 readonly 修饰的意思。

![](./images/a74b89eedeae56750b0fb5042ba1f342.webp )
...
![](./images/331cfb7b537732553c1411e66385c2b8.webp )

（准确来说叫元组，元组是元素个数固定的数组）

那我们只要在传入 Zip 之前，用 Mutable 去掉 readonly 就可以了：

![](./images/eb97437d159f787f6965bda71b1de8b5.webp )

再来试一下：

![](./images/35e0d83a765798e42b2214e26b682c84.webp )

大功告成！现在返回值的类型就对了。

但还有个问题，如果不是直接传入字面量，是推导不出字面量类型的，这时候貌似就不对了：

![](./images/f9eb9bc56def74eac715ac8d29c8870e.webp )

可我们不都声明重载类型了么？

如果推导不出字面量类型，应该匹配这个呀：

![](./images/264c418fcaf58a53819da04d49516882.webp )

但实际上它匹配的还是第一个：

![](./images/d1b8345e3cc9e2327a693980c0dc92b0.webp )

这时候其实只要调换下两个函数类型的顺序就可以了：

![](./images/3a152be7375d7c9c7d2c9a606f345778.webp )

![](./images/f26423e117459a48382427f343401a43.webp )

这时字面量参数的情况依然也是对的：

![](./images/66d31892986ff05fa5b56a6072724d93.webp )
...
![](./images/c884dbb7c7fccf6780d4b8aeac11c342.webp )
